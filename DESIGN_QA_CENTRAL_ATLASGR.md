...
- **`jsx-a11y/label-has-associated-control` (74), `click-events-have-key-events` (26),
  ...
  **Atualização (Onda 8):** das 74 ocorrências originais de `label-has-associated-control`, 46 já
  tinham sido fechadas em ondas anteriores (rodadas de remediação não documentadas individualmente
  aqui); as 28 restantes foram fechadas nesta rodada — ver DQA-21. `npm run lint` confirma 0
  ocorrências dessa regra específica hoje. A regra continua em `warn` no `eslint.config.mjs` (não
  promovida a `error`) porque as outras 5 regras do mesmo grupo (`click-events-have-key-events`,
  `no-static-element-interactions`, `no-noninteractive-element-interactions`, `no-autofocus`,
  `media-has-caption`) ainda têm ocorrências reais abertas, fora do escopo desta rodada.
...
| DQA-21 | 28 ocorrências em 9 telas (Atividades, Auth×3, Chatbook, Bitrix Import, Prospecção×3) | `jsx-a11y/label-has-associated-control` (rebaixada a `warn` em `eslint.config.mjs`) | 28 `<label>` sem `htmlFor`/`id` associado ao controle (inputs de data/hora/texto, `<select>`s de filtro, grupo de botões de temperatura/toggle no Bitrix Import) | Média (débito de lint conhecido) | Onda 8: todas as 28 corrigidas — `htmlFor`/`id` nos pares label+input/select; grupos de botão (não são um único controle nativo) convertidos de `<label>` pra `<span id>` + `role="group" aria-labelledby`; um checkbox com texto aninhado profundo recebeu `aria-label` explícito. `npm run lint` confirma 0 ocorrências restantes da regra | ✅ Corrigido |
| DQA-22 | Variante `default` de `ui/Button.tsx` (botão primário mais reutilizado do app) + `AIEmailGenerator.tsx`, `ClockCalendarWidget.tsx` (dia selecionado) | Texto branco sobre gradiente `from-brand to-brand-2`/`bg-brand` puro | Mesma classe de bug do DQA-19, não coberta por ele: `--brand-2` puro (extremo do gradiente) dá ~2.9:1 (AtlasGR)/~3.6:1 (TotalTrac), abaixo de 4.5:1 — presente no botão primário padrão do design system inteiro, não descoberto pelo `axe-core` porque a ferramenta não avalia contraste sobre `background-image`/gradiente. Achado por auditoria manual (Onda 8) | **Séria (WCAG AA)** | Novo token `--color-brand-2-active` (mesma fórmula de `--color-brand-active`, `color-mix` 75%/preto); `Button.tsx` default e os 3 usos apontados passaram a usar `brand-active`/`brand-2-active`. `hover:brightness-110` do `Button` removido (levava o extremo já escurecido de volta a ~4.1:1) — feedback de hover mantido só via shadow/`active:scale-[0.97]` (já existente) | ✅ Corrigido |
| DQA-23 | `useMagnetic()` em `src/lib/motion.ts`; `AtlasOrb.tsx` (esfera 3D decorativa) | `prefers-reduced-motion` | `useMagnetic` chamava `useReducedMotion()` mas nunca checava o valor no `onPointerMove` (diferente de `useTilt`, que já fazia essa guarda) — o efeito magnético continuava puxando o elemento com a preferência ativa. `AtlasOrb` (react-three-fiber/drei `Float`+`MeshDistortMaterial`+`Sparkles`) gira/distorce continuamente via `useFrame`, fora do alcance do `@media (prefers-reduced-motion)` global de `globals.css` (que só zera `animation`/`transition` CSS) | Média (WCAG 2.3.3) | `useMagnetic` ganhou a mesma guarda `if (reduceMotion) return`. `AtlasOrb` zera as amplitudes de movimento (`speed`/`rotationIntensity`/`floatIntensity`/`distort`) quando `useReducedMotion()` é verdadeiro, mantendo a peça visível e estática em vez de desmontar o Canvas | ✅ Corrigido |

#!/usr/bin/env python3
"""Gera 1 relatório HTML de fit ICP/RNTRC/PIC por município, para todo o Brasil.

Generaliza o relatório de referência de Campinas/SP para qualquer município do país,
processando os CSVs normalizados por UF, sem depender de Postgres/Docker.

Semântica:
- universo setorial: empresa ATIVA na Receita Federal cujo CNAE principal ou secundário
  bate com o Tier A da taxonomia `icp_taxonomy.v1.json`;
- RNTRC é cruzado por CNPJ e preservado como evidência separada;
- `icpFit=FIT_ESTRUTURAL_CONFIRMADO` somente quando RNTRC está ATIVO;
- `FIT_SETORIAL_POTENCIAL` permanece potencial quando RNTRC não está ativo;
- o bucket comercial `wave1` é uma pré-priorização e NÃO é sinônimo de ICP confirmado;
- Tier A é setorial e a taxonomia v1 permanece NÃO CALIBRADA.

Uso:
  python scripts/market_intelligence/generate_municipality_fit_reports.py --uf SP --dry-run
  python scripts/market_intelligence/generate_municipality_fit_reports.py --uf SP --only-fit
  python scripts/market_intelligence/generate_municipality_fit_reports.py --uf SP --only-wave1
  python scripts/market_intelligence/generate_municipality_fit_reports.py
"""
...
def matched_tier_a_cnae(
    cnae_principal: str,
    cnaes_secundarios_raw: str,
    prefixes: list[str],
) -> tuple[str, str, str] | None:
    """Retorna (cnaeFit, fitEvidence, descrição) quando há aderência setorial Tier A."""
...
def load_rntrc_map() -> dict[str, tuple[str, str, str]]:
    ...
    with RNTRC_CSV.open(encoding="utf-8", newline="") as fh:
        for row in csv.DictReader(fh):
            result[row["cnpj"]] = (row["rntrcStatus"], row["rntrcNumber"], row["rntrcType"])
    return result
...
def classify_fit_and_bucket(rntrc_status: str, pic_hypothesis: str) -> tuple[str, str, str]:
    """Separa fit estrutural de bucket comercial; nenhuma dimensão implica a outra."""
    if rntrc_status == "NAO":
        bucket = "nurture"
    elif pic_hypothesis == "PIC_NAO_INFERIVEL":
        bucket = "research"
    else:
        bucket = "wave1"

    if rntrc_status == "ATIVO":
        return bucket, "FIT_ESTRUTURAL_CONFIRMADO", "ALTO_CONFIRMADO_RNTRC"
    return bucket, "FIT_SETORIAL_POTENCIAL", "ALTO_SETORIAL"


def should_include(selection_mode: str, bucket: str, icp_fit: str) -> bool:
    if selection_mode == "confirmed":
        return icp_fit == "FIT_ESTRUTURAL_CONFIRMADO"
    if selection_mode == "wave1":
        return bucket == "wave1"
    return True


def build_message_table() -> dict[tuple[str, str], dict[str, str]]:
    """Extrai (cnaeFit, rntrcMatch) -> mensagem do relatório de referência de Campinas."""
    ...
    for key, rows in groups.items():
        ...
        for field in fields:
            ...
            if len(values) > 1:
                inconsistent += 1
                print(
                    f"AVISO: campo '{field}' inconsistente para {key} ({dict(values)}); "
    ...
    if inconsistent:
        print(f"AVISO: {inconsistent} campo(s) inconsistentes na tabela de mensagens extraída.", file=sys.stderr)
    print(f"Tabela de mensagens: {len(table)} combinações (cnaeFit, rntrcMatch) extraídas de Campinas.", file=sys.stderr)
    return table
...
def build_template() -> tuple[str, str]:
    """Extrai prefixo/sufixo do HTML de referência e instala marcadores municipais."""
    content = REFERENCE_HTML.read_text(encoding="utf-8")
...
def render_report(
    ...
    total = len(rows)
    confirmado = sum(1 for row in rows if row["icpFit"] == "FIT_ESTRUTURAL_CONFIRMADO")
    potencial = sum(1 for row in rows if row["icpFit"] == "FIT_SETORIAL_POTENCIAL")
    rntrc_ativo = sum(1 for row in rows if row["rntrcMatch"] == "ATIVO")
    interestadual = sum(1 for row in rows if row["interestadualCnae"] == "SIM")
    invalidos = sum(1 for row in rows if row["cnpjValido"] != "SIM")
    wave1 = sum(1 for row in rows if row["_bucket"] == "wave1")
    research = sum(1 for row in rows if row["_bucket"] == "research")
    nurture = sum(1 for row in rows if row["_bucket"] == "nurture")
    ...
    <h2>Processo de marketing preparado</h2>
    <div class="profiles">
      <article class="profile"><h3>Onda 1 — validar PIC 1</h3><p><b>{fmt_int(wave1)} contas</b> no bucket wave1 de pré-priorização; validar contexto e fit antes do outbound.</p></article>
    ...
    <p><b>Status:</b> preparado, não disparado. Bucket comercial e fit estrutural são dimensões diferentes. Este HTML não contém nomes de pessoas, telefones ou e-mails; antes do contato é obrigatório confirmar finalidade, proveniência e opt-out.</p>
  </section>"""
    meta_block = (
        ...
        f'<div><b>Tier setorial:</b> A / NÃO CALIBRADO; não equivale a ICP final</div>'
    ...
    data_rows = [{key: value for key, value in row.items() if not key.startswith("_")} for row in rows]
...
def process_uf(
    ...
    with gzip.open(csv_path, mode="rt", encoding="utf-8", newline="") as fh:
        for row in csv.DictReader(fh):
            ...
            bucket, icp_fit, fit_classe = classify_fit_and_bucket(rntrc_status, message["picHypothesis"])
            ...
            out_row = {
                ...
                "tierSetorial": "A",
                "calibrationStatus": "NAO_CALIBRADO",
            ...
            if not should_include(selection_mode, bucket, icp_fit):
                continue
            by_municipio[row["municipioIbge"]].append(out_row)
            total_companies += 1

    if dry_run:
        for ibge, rows in sorted(by_municipio.items(), key=lambda item: -len(item[1]))[:5]:
            print(f"[{uf}] {rows[0]['municipioNome']} ({ibge}): {len(rows)} empresas Tier A setorial", file=sys.stderr)
        print(f"[{uf}] total: {len(by_municipio)} municípios, {total_companies} empresas", file=sys.stderr)
    ...
    for ibge, rows in by_municipio.items():
        ...
        index_entries.append({
            ...
            "slug": slug,
            "total": len(rows),
            "confirmed": sum(1 for row in rows if row["icpFit"] == "FIT_ESTRUTURAL_CONFIRMADO"),
            "wave1": sum(1 for row in rows if row["_bucket"] == "wave1"),
        })
    print(f"[{uf}] {written} relatórios de município escritos em {out_dir}", file=sys.stderr)
    return len(by_municipio), total_companies


def fmt_br(value: int) -> str:
    return f"{value:,}".replace(",", ".")


def build_index_html(index_entries: list[dict], selection_mode: str, generated_at: str) -> str:
    by_uf: dict[str, list[dict]] = defaultdict(list)
    for entry in index_entries:
        by_uf[entry["uf"]].append(entry)

    total_municipios = len(index_entries)
    total_empresas = sum(entry["total"] for entry in index_entries)
    total_confirmed = sum(entry["confirmed"] for entry in index_entries)
    total_wave1 = sum(entry["wave1"] for entry in index_entries)

    uf_cards: list[str] = []
    uf_sections: list[str] = []
    for uf in sorted(by_uf):
        entries = sorted(by_uf[uf], key=lambda entry: -entry["total"])
        uf_total = sum(entry["total"] for entry in entries)
        uf_cards.append(
            f'<a class="uf-card" href="#uf-{uf}"><strong>{uf}</strong>'
            f'<span>{len(entries)} município(s)</span><span>{fmt_br(uf_total)} empresas</span></a>'
        )
        rows_html = "".join(
            f'<tr><td><a href="{uf}/{entry["slug"]}.html">{entry["municipio"]}</a></td>'
            f'<td>{fmt_br(entry["total"])}</td><td>{fmt_br(entry["confirmed"])}</td>'
            f'<td>{fmt_br(entry["wave1"])}</td></tr>'
        ...
        uf_sections.append(
            ...
            f'<th>Fit estrutural confirmado</th><th>Bucket wave1 (pré-priorização)</th></tr></thead>'
            f'<tbody>{rows_html}</tbody></table></div></section>'
        )

    if selection_mode == "confirmed":
        escopo = "Somente FIT_ESTRUTURAL_CONFIRMADO: CNAE aderente + RNTRC oficial ATIVO."
    elif selection_mode == "wave1":
        escopo = (
            "Somente bucket wave1 de pré-priorização; pode incluir RNTRC PENDENTE e "
            "não equivale a ICP confirmado."
        )
    else:
        escopo = "Todo o universo setorial Tier A não calibrado (wave1 + pesquisa + nurture)."
...
<head>
  ...
  <title>AtlasGR | Mapa nacional de universo candidato por município</title>
  <style>
...
<body>
<header>
  <h1>Mapa nacional de universo candidato ICP/RNTRC/PIC — AtlasGR</h1>
  <p>{escopo} Gerado em {generated_at}. Bucket comercial e fit estrutural são dimensões separadas.</p>
</header>
<main>
  <section class="cards">
    ...
    <div class="card"><small>Fit estrutural confirmado</small><strong>{fmt_br(total_confirmed)}</strong></div>
    <div class="card"><small>Bucket wave1 (pré-priorização)</small><strong>{fmt_br(total_wave1)}</strong></div>
  </section>
...
def main() -> int:
    ...
    parser.add_argument("--uf", action="append", dest="ufs", choices=ALL_UFS, help="Repetir para várias UFs; omitir para rodar as 27.")
    parser.add_argument("--dry-run", action="store_true", help="Só reporta contagens, não escreve HTML.")
    scope = parser.add_mutually_exclusive_group()
    scope.add_argument(
        "--only-fit",
        action="store_true",
        help="Só inclui FIT_ESTRUTURAL_CONFIRMADO (CNAE aderente + RNTRC ATIVO).",
    )
    scope.add_argument(
        "--only-wave1",
        action="store_true",
        help="Só inclui bucket wave1 de pré-priorização; pode conter RNTRC PENDENTE.",
    )
    parser.add_argument("--output-dir", type=Path, default=OUTPUT_ROOT, help="Pasta de saída (default: .cache/market-intelligence/reports/Brasil).")
    args = parser.parse_args()

    selection_mode = "confirmed" if args.only_fit else "wave1" if args.only_wave1 else "all"
    ufs = args.ufs or ALL_UFS
    prefixes = load_tier_a_prefixes()
    print(f"Prefixos Tier A setorial (taxonomia não calibrada): {prefixes}", file=sys.stderr)
    ...
    start = datetime.now()
    for uf in ufs:
        municipios, empresas = process_uf(
            uf,
            prefixes,
            rntrc_map,
            message_table,
    ...
    if not args.dry_run and index_entries:
        ...
        (args.output_dir / "index.html").write_text(
            build_index_html(index_entries, selection_mode, generated_at),
            encoding="utf-8",
        )
        print(f"index.html escrito em {args.output_dir / 'index.html'}", file=sys.stderr)

    print("=" * 60, file=sys.stderr)
    print(f"Modo de seleção: {selection_mode}", file=sys.stderr)
    print(f"UFs processadas: {len(ufs)}", file=sys.stderr)
    print(f"Municípios com relatório: {total_municipios}", file=sys.stderr)
    print(f"Empresas processadas: {total_empresas}", file=sys.stderr)
    print(f"Tempo total: {elapsed:.1f}s", file=sys.stderr)
    if fallback_counter:
        print(
            f"Combinações (cnaeFit,rntrcMatch) sem mensagem observada em Campinas: {len(fallback_counter)}",
            file=sys.stderr,
        )

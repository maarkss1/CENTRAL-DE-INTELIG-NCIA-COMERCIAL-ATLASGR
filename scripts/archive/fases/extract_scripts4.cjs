        ...
        if (obj.includes('#Requires -Version 5.1')) {
          ...
          for (let i = 1; i < parts.length; i++) {
            ...
            if (script.includes('Write-FileFromContent -RelativePath "src/modules/crm/application/')) {
              fs.writeFileSync('C:\\Users\\USER\\Documents\\GitHub\\PROSPECTOR-ATLAS\\FASE-10-APPLICATION.ps1', script);
              found_10 = true;
              console.log('Found FASE 10');
            }
            
            if (script.includes('FASE 11 — INFRASTRUCTURE LAYER ENTERPRISE')) {
              fs.writeFileSync('C:\\Users\\USER\\Documents\\GitHub\\PROSPECTOR-ATLAS\\FASE-11-INFRASTRUCTURE.ps1', script);
              found_11 = true;

-- Etapa 2: universo empresarial versionado e separado do CRM Company.
...
CREATE TABLE "MarketIntelligenceCompany" (
  ...
  CONSTRAINT "MarketIntelligenceCompany_icpScore_check" CHECK ("icpScore" IS NULL OR "icpScore" BETWEEN 0 AND 100),
  CONSTRAINT "MarketIntelligenceCompany_cnpj_check" CHECK ("cnpj" ~ '^[A-Z0-9]{12}[0-9]{2}$')
);
...
CREATE INDEX "MarketIntelligenceCompany_datasetId_municipioIbge_situacao_idx" ON "MarketIntelligenceCompany"("datasetId","municipioIbge","situacaoCadastral");
...
CREATE INDEX "MarketIntelligenceCompany_razaoSocialSearch_trgm_idx" ON "MarketIntelligenceCompany" USING GIN ("razaoSocialSearch" gin_trgm_ops);
CREATE INDEX "MarketIntelligenceCompany_nomeFantasiaSearch_trgm_idx" ON "MarketIntelligenceCompany" USING GIN ("nomeFantasiaSearch" gin_trgm_ops);
CREATE INDEX "MarketIntelligenceCompany_cnaesSecundarios_gin_idx" ON "MarketIntelligenceCompany" USING GIN ("cnaesSecundarios" jsonb_path_ops);
...
ALTER TABLE "MarketIntelligenceCompany" ADD CONSTRAINT "MarketIntelligenceCompany_datasetId_fkey"
  FOREIGN KEY ("datasetId") REFERENCES "MarketIntelligenceDataset"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- A API autenticada lê o catálogo global; somente o importador com app.bypass_rls=on escreve.
...
CREATE POLICY market_intelligence_read ON "MarketIntelligenceDataset" FOR SELECT USING (
  current_setting('app.current_tenant_id', TRUE) <> '' OR current_setting('app.bypass_rls', TRUE) = 'on'
);

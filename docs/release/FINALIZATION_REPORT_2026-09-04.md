...
| 12 | **P1 (segurança, achado do CI real)** | `discover_latest_ciot()` em `public/tools/hub-inteligencia-marketing/etl_mdfe_atlas.py` (um dos arquivos restaurados pelo achado #1) extrai `resource["url"]` inteiro (host incluso) direto da resposta JSON da API CKAN pública da ANTT e passa pra `urllib.request.urlopen` — SSRF real, achado pelo CodeQL no CI oficial do GitHub (não pelo sweep local desta sessão, que não roda CodeQL) | URL só era filtrada por um regex de substring (`_ciots\.csv$` em algum lugar da string) antes de ser buscada — não valida esquema nem host | `assert_allowed_source_url()`: valida `scheme == 'https'` e `hostname` contra um allowlist fixo (`dados.antt.gov.br`) antes de qualquer fetch — mesmo padrão de allowlist já usado em `fetchWithTimeout`/`safeFetch` (PR #339) | `a4827ab` |
...
| B12 | **P1 (segurança)** | SSRF real em `etl_mdfe_atlas.py` (URL de fetch vinda inteira de resposta de API externa, sem validar host) | `resource["url"]` da API CKAN da ANTT direto pro `urllib.request.urlopen`, só filtrada por regex de substring | Allowlist de host fixo antes do fetch, mesmo padrão de `fetchWithTimeout`/`safeFetch` | CodeQL real do CI (PR #344) — achado real, não teórico | ✅ Corrigido (`a4827ab`) |

...
| 13 | **P1 (segurança, achado do CI real, confirmado via SARIF baixado)** | DOM XSS real em `public/tools/social-selling/Atlas GR Pipeline.html` (também restaurado pelo achado #1) — `x.temperatura` concatenado direto no `innerHTML` sem `escapeHtml()`, e `fmtDateBR()` com um fallback que devolvia a data bruta sem escapar quando malformada | Ambos os valores vêm de `<input>` client-side (persistidos em `localStorage`, sem validação de servidor) — um bypass da UI (devtools, extensão comprometida, futura importação) injetaria HTML/script real no `innerHTML` | `escapeHtml()` aplicado em `x.temperatura` (faltava, inconsistente com os campos vizinhos) e movido pra dentro de `fmtDateBR()` (cobre os 3 call sites de uma vez) — comportamento idêntico pra qualquer data/temperatura válida | `27f1aab` |
...
| B13 | **P1 (segurança)** | DOM XSS real em `Atlas GR Pipeline.html` (`temperatura` sem escape; `fmtDateBR` com fallback sem escape) | Dado de `<input>` client-side sem validação de servidor, concatenado direto em `innerHTML` | `escapeHtml()` no campo faltante + movido pra dentro de `fmtDateBR()` | SARIF real baixado dos artefatos do CI (não suposição) — `js/xss-through-dom`, severidade `high` | ✅ Corrigido (`27f1aab`) |

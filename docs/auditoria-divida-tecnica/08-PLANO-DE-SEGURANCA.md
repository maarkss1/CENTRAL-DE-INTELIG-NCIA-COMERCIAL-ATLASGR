...
- **Status (2026-08-01): código pronto, rollout em produção pendente de execução do usuário.**
  - Criado `scripts/db/create-app-role.sql` (idempotente) — cria o papel `prospector_app` (`NOSUPERUSER`), garante `CREATE EXTENSION vector`, e faz `REASSIGN OWNED BY` pra transferir a posse dos objetos existentes do superusuário de bootstrap pro papel restrito (é a posse, não só o `GRANT`, que faz `FORCE ROW LEVEL SECURITY` valer).

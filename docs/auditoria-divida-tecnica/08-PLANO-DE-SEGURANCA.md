...
- **Status (2026-08-01): código pronto, rollout em produção pendente de execução do usuário.**
  - Criado `scripts/db/create-app-role.sql` (idempotente) — cria o papel `prospector_app` (`NOSUPERUSER`), garante `CREATE EXTENSION vector`, e transfere a posse do schema `public` e de cada tabela/sequência existente pro papel restrito (é a posse, não só o `GRANT`, que faz `FORCE ROW LEVEL SECURITY` valer). Não usa `REASSIGN OWNED BY CURRENT_USER` — testado localmente e falha sempre (mesmo em banco vazio) com erro `2BP01`, porque o superusuário de bootstrap também é dono de objetos exigidos pelo sistema (a extensão `plpgsql`, instalada automaticamente em todo banco novo); a versão atual escopa a transferência em `pg_tables`/`pg_sequences` de `public` pra evitar isso. Validado localmente (Postgres 16, banco vazio e com tabelas pré-existentes) ponta a ponta: `prospector_app` consegue criar tabelas (migrations continuam funcionando) e fica de fato sujeito a `FORCE ROW LEVEL SECURITY` (leitura sem `app.current_tenant_id` setado retorna 0 linhas).

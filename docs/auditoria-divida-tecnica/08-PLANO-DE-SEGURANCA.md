...
- **Evidência:** `k8s/postgres-statefulset.yaml` e `docker-compose.yml` só configuram `POSTGRES_USER`/`POSTGRES_PASSWORD` — o mecanismo de bootstrap do Postgres torna esse usuário **superusuário**, e não existia em nenhum lugar do repositório a criação de um papel de aplicação restrito (`NOSUPERUSER`) com `GRANT` explícito. `DATABASE_URL` em todo ambiente (local, docker-compose, k8s) apontava para esse mesmo papel.
...
- **Impacto real:** **todas** as políticas RLS já criadas neste projeto — as de `Company`/`Contact`/`Lead`/`Activity`/`User`/`Organization` (migration `20260722020322_enable_rls`), `Document`/`DocumentChunk` (`20260731170000`), `Automation`/`Notification`/`AILog` (`20260731200000`), e as novas de `KnowledgeChunk`/`Prompt`/`AgentMemory`/`Prospect`/`AIPendingAction` desta sessão — não ofereciam nenhuma proteção real. O item L5 deste documento ("o padrão de isolamento de tenant no domínio central é sólido") descrevia corretamente o filtro **de aplicação** (`getTenantPrisma`, `organizationId` explícito nas queries), que é o que de fato protege hoje — a camada de RLS no banco era, na prática, decorativa.
- **Status (2026-08-01): código pronto, rollout em produção pendente de execução do usuário.**
  - Criado `scripts/db/create-app-role.sql` (idempotente) — cria o papel `prospector_app` (`NOSUPERUSER`), garante `CREATE EXTENSION vector`, e transfere a posse do schema `public` e de cada tabela/sequência existente pro papel restrito (é a posse, não só o `GRANT`, que faz `FORCE ROW LEVEL SECURITY` valer). Duas armadilhas reais encontradas e corrigidas ao validar contra Postgres de verdade (não só lendo a doc):
    1. `REASSIGN OWNED BY CURRENT_USER` falha sempre (erro `2BP01`), mesmo em banco vazio, porque o superusuário de bootstrap também é dono de objetos exigidos pelo sistema (a extensão `plpgsql`, instalada automaticamente em todo banco novo) — corrigido escopando a transferência em `pg_tables`/`pg_sequences` de `public`.
    2. A interpolação de variável do `psql` (`:'app_password'`) **não acontece dentro de blocos `DO $$ ... $$`** — é puramente lexical e para de funcionar assim que entra num bloco dollar-quoted, então o texto chegava literal no servidor e virava erro de sintaxe (`exit code 3`, reproduzido de verdade contra `pgvector/pgvector:pg15` via Docker local antes de existir uma explicação só por leitura de doc). Corrigido trocando por `SELECT format(...) ... \gexec`, que mantém a interpolação num `SELECT` de nível superior, onde ela funciona.
    Validado ponta a ponta contra a imagem e o `psql` reais que a CI usa (`pgvector/pgvector:pg15`, via Docker local): bootstrap roda de forma idempotente (roda 2x, a segunda troca a senha via `ALTER ROLE`), `prospector_app` loga com a senha nova, cria tabelas normalmente (migrations continuam funcionando), e fica de fato sujeito a `FORCE ROW LEVEL SECURITY` (leitura sem `app.current_tenant_id` setado retorna 0 linhas; com o contexto certo, retorna as linhas do tenant).
  - Criado `scripts/db/bootstrap-app-role.sh` — wrapper pra rodar o script acima contra qualquer ambiente (staging, produção) via `psql`, lendo a senha do papel de `APP_DB_PASSWORD`.
  - `docker-compose.yml` agora roda o bootstrap automaticamente via `docker-entrypoint-initdb.d` em volume novo, e `.env.example` já aponta `DATABASE_URL` pro papel `prospector_app`. **Atenção:** isso só se aplica a volumes Docker novos — quem já tem `atlas_pgdata` de antes precisa rodar `scripts/db/bootstrap-app-role.sh` manualmente contra o volume existente (não é destrutivo: só troca posse, não apaga dado).
  - `.github/workflows/ci.yml` e `production.yaml` agora bootstrapam o papel e rodam `prisma migrate deploy`/testes de integração já como `prospector_app`, não mais como o superusuário do container de teste — os testes de RLS/tenant-isolation passam a validar as condições reais de produção.
  - **Pendente — precisa de decisão/execução do usuário, fora do alcance deste ambiente (mexe em infraestrutura viva):**
    1. Definir uma senha real para `APP_DB_PASSWORD` em produção (nunca a de exemplo do `docker-compose.yml`/CI) e guardá-la como segredo.
    2. Rodar `scripts/db/bootstrap-app-role.sh` contra o Postgres de produção (cria o papel, transfere posse — não apaga dados, mas é uma mudança de privilégios num banco vivo; recomenda-se rodar primeiro em staging/homolog).
    3. Adicionar/editar a chave `DATABASE_URL` dentro do Secret `prospector-secrets` do k8s pra apontar pro papel `prospector_app` em vez do superusuário (`k8s/api-deployment.yaml` já consome esse Secret via `envFrom`; `k8s/postgres-statefulset.yaml` continua usando `POSTGRES_USER`/`POSTGRES_PASSWORD` só pro bootstrap do container do banco, sem mudança necessária).
    4. Fazer o deploy do `api-server` com o novo `DATABASE_URL` e confirmar que a aplicação continua funcionando sem privilégios de superusuário.

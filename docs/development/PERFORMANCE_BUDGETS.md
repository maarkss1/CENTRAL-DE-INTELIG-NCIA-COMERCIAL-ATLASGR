...
## 2. Latência de endpoints críticos (`tests/load/k6-api.js`)
...
2. Instala dependências, gera Prisma Client, roda migrations, monta `.env.test`.
...
4. Instala o binário oficial do k6 (versão pinada, sem Docker) e roda
   `k6 run tests/load/k6-api.js` contra a app local, exportando o resumo
   (`k6-latency-report.json`) como artefato de 90 dias.

Dispara em PR que toca `server.ts`, `src/bootstrap/**` ou `tests/load/**`, mais diariamente
(`schedule`, 06:00 UTC) para publicar a tendência mesmo sem PR, e sob demanda
(`workflow_dispatch`). **Não é o required check `build`** do repositório — mesma decisão de
`public-assets-budget.yml` do ITEM-05: falha visível no PR, sem arriscar quebrar o gate de release
por um teste de carga instável.

### Débito derivado (não resolvido neste item)

`GET /health/ready` é o endpoint mais próximo de uma dependência externa real que dá para
exercitar **sem autenticação**. `docs/SRE.md` 1.2 cita como exemplo real "leitura de CRM,
listagem de contatos" — essas rotas exigem sessão autenticada (Better Auth) e não são exercitadas
por este k6 script. Simular login dentro do k6 (ou usar um token de service account de teste) para
cobrir uma rota transacional autenticada de verdade é debito real, registrado aqui em vez de ser
forjado só para preencher a métrica (proibição explícita das regras globais de execução deste
pacote de remediação: "não usar mocks para fingir integração real onde o requisito exige
integração real").

...
| 3 | `test-gemini.ts:2`, `test-gemini-quota.ts:2` | `40242d7e`, `db20c1f3` | **Achado real, JÁ REMEDIADO E REMOVIDO DO HISTÓRICO.** Chave de API real do Google Gemini (`AQ.Ab8RN6Kc...`, formato Google AI Studio) hardcoded em 2 scripts de teste ad-hoc. Os arquivos não existiam mais na árvore de trabalho, mas a chave era recuperável no histórico. Confirmado que `@google/genai` não é dependência do `package.json` atual e nenhum caminho de produção instancia `GoogleGenAI` diretamente — não havia indício de uso corrente pelo produto. | **Dupla remediação, 2026-09-05:** (1) Rotacionada — confirmado diretamente pelo dono do repositório; runbook `docs/security/runbooks/ROTATE_GEMINI_API_KEY.md` marcado "Concluído", fingerprints suprimidos em `.gitleaksignore`. (2) **Removida fisicamente do histórico de `main`** via `git filter-repo --path test-gemini.ts --path test-gemini-quota.ts --invert-paths` + force-push (mesma operação que removeu `backups/prospector-*.dump`, ver `docs/security/runbooks/DECIDE_GIT_HISTORY_REWRITE.md` → "Decisão revista — Caminho B executado"). `git rev-list --objects main` (sem `--all`) confirma zero blobs desses 2 arquivos hoje. As outras 81 branches remotas do repositório (não reescritas, depois deletadas numa limpeza separada) ainda continham o literal até serem removidas. |

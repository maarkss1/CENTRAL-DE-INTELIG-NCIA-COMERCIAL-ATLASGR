...
| 3 | `test-gemini.ts:2`, `test-gemini-quota.ts:2` | `40242d7e`, `db20c1f3` | **Achado real, ABERTO.** Chave de API real do Google Gemini (`AQ.Ab8RN6Kc...`, formato Google AI Studio) hardcoded em 2 scripts de teste ad-hoc. Os arquivos não existem mais na árvore de trabalho, mas a chave é recuperável no histórico. Confirmado que `@google/genai` não é dependência do `package.json` atual e nenhum caminho de produção instancia `GoogleGenAI` diretamente — não há indício de uso corrente pelo produto, mas isso não invalida a chave do lado do Google. | **Runbook criado:** `docs/security/runbooks/ROTATE_GEMINI_API_KEY.md`. Pendente de execução humana (revogar no Google AI Studio) — fora do alcance de qualquer sessão automatizada. |
...
- R9 (`FINALIZATION_REPORT_2026-09-04.md`) dizia "45 segredos... avaliação/rotação é decisão do
  dono do produto", sem detalhar quais eram nem quantos exigiam ação real. Esta triagem reduz isso
  a **1 ação humana pendente real** (revogar a chave Gemini) e **1 confirmação recomendada** (valor

...
async function syncLeadToBitrix(
    ...
    try {
        ...
        if (lead.bitrixLeadId) {
            await callBitrix(webhookUrl, 'crm.lead.update', { id: lead.bitrixLeadId, fields }, { correlationId });
            await prisma.lead.update({ where: { id: lead.id }, data: { bitrixSyncStatus: 'synced', bitrixSyncError: null, bitrixSyncedAt: new Date() } });
            await logSync({ organizationId, connectionId, direction: 'outbound', entityType: 'lead', leadId, bitrixRecordId: lead.bitrixLeadId, status: 'success', correlationId });
        ...
        if (!claimed) {
            throw new AppError('Este lead já está sendo sincronizado com o Bitrix24 — tente novamente em alguns segundos.', 409);
        }
        ...
        const existingBitrixId = await findDuplicateBitrixLeadId(webhookUrl, { email, phone }, correlationId);

        if (existingBitrixId) {
            await callBitrix(webhookUrl, 'crm.lead.update', { id: existingBitrixId, fields }, { correlationId });
            await prisma.lead.update({ where: { id: lead.id }, data: { bitrixLeadId: existingBitrixId, bitrixSyncStatus: 'synced', bitrixSyncError: null, bitrixSyncedAt: new Date() } });
            await logSync({ organizationId, connectionId, direction: 'outbound', entityType: 'lead', leadId, bitrixRecordId: existingBitrixId, status: 'success', correlationId });
            logger.info({ correlationId, organizationId, leadId, bitrixLeadId: existingBitrixId }, '[bitrix] Duplicidade evitada — lead local vinculado a um Lead Bitrix já existente (crm.duplicate.findbycomm)');
            return { bitrixLeadId: existingBitrixId };
        }

        const { result: newId } = await callBitrix<{ result: string }>(webhookUrl, 'crm.lead.add', { fields }, { correlationId });
        await prisma.lead.update({ where: { id: lead.id }, data: { bitrixLeadId: String(newId), bitrixSyncStatus: 'synced', bitrixSyncError: null, bitrixSyncedAt: new Date() } });
        ...
        await logSync({ organizationId, connectionId, direction: 'outbound', entityType: 'lead', leadId, bitrixRecordId: String(newId), status: 'success', correlationId });
        ...
        await prisma.lead.update({ where: { id: lead.id }, data: { bitrixSyncStatus: 'failed', bitrixSyncError: errorMessage } }).catch((updateErr) => {
...
async function findDuplicateBitrixLeadId(webhookUrl: string, contact: { email: string | null; phone: string | null }, correlationId: string): Promise<string | null> {
    for (const [type, value] of [['EMAIL', contact.email], ['PHONE', contact.phone]] as const) {
        if (!value) continue;
        try {
            const data = await callBitrix<BitrixDuplicateFindResult>(webhookUrl, 'crm.duplicate.findbycomm', { type, values: [value], entity_type: 'LEAD' }, { correlationId, maxAttempts: 1 });
            const found = data.result?.LEAD?.[0];
            if (found) return found;
        } catch (err) {
            logger.debug({ err, type, correlationId }, '[bitrix] crm.duplicate.findbycomm indisponível ou sem permissão — seguindo sem checagem de duplicidade');
        }
    }
    return null;
}
...
export async function pushLeadToBitrix(organizationId: string, leadId: string): Promise<void> {
    ...
    try {
        ...
        logger.warn({ err, correlationId, organizationId, leadId }, '[bitrix] Falha ao enviar lead automaticamente — motivo salvo em Lead.bitrixSyncError e BitrixSyncLog');
    }
}
...
export async function exportLeadToBitrixNow(
    ...
    if (leadId && leadId !== 'all') {
        const result = await syncLeadToBitrix(organizationId, resolvedConnectionId, webhookUrl, leadId, overrides);
        await AuditService.log({ action: 'EXPORT', entity: 'Lead', entityId: leadId, tenantId: organizationId, afterState: { bitrixLeadId: result.bitrixLeadId } });
        return result;
    }
    ...
    await AuditService.log({ action: 'EXPORT', entity: 'Lead', tenantId: organizationId, afterState: { exportedCount, skippedCount, total: leads.length } });

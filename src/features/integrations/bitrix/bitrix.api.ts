...
export const bitrixApi = {
    ...
    listLeads: async (connectionId: string, search?: string): Promise<{ leads: BitrixLeadItem[]; total: number }> => {
        const query = new URLSearchParams({ connectionId });
        if (search) query.set('search', search);
        const res = await api.get<{ data: { leads: BitrixLeadItem[]; total: number } }>(`/api/bitrix/leads?${query.toString()}`);
        return res.data;
    },

    listDeals: async (connectionId: string, search?: string): Promise<{ deals: BitrixDealItem[]; total: number }> => {
        const query = new URLSearchParams({ connectionId });
        if (search) query.set('search', search);
        const res = await api.get<{ data: { deals: BitrixDealItem[]; total: number } }>(`/api/bitrix/deals?${query.toString()}`);
        return res.data;
    },
    ...
    importRecentBitrixLeads: async () => {
        return api.post<{ success: boolean; data: { imported: number; skipped: number } }>('/api/crm/leads/import/bitrix24', {});
    },

    exportLead: async (leadId: string, connectionId?: string) => {
        return api.post<{ success: boolean; data: { bitrixLeadId: string } }>(`/api/bitrix/leads/${leadId}/export`, { connectionId });
    },

    exportLeadsBatch: async (leadIds?: string[], connectionId?: string) => {
        return api.post<{ success: boolean; data: { exportedCount: number; skippedCount: number } }>('/api/bitrix/leads/export-batch', { leadIds, connectionId });
    },
};

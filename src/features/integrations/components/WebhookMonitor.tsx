...
import { Activity, RefreshCw, ArrowDownLeft, ArrowUpRight, Clock, Search, AlertTriangle } from 'lucide-react';
...
export function WebhookMonitor() {
    ...
    const loadLogs = useCallback(async () => {
        ...
        try {
            ...
            setLoadError(e instanceof Error ? e.message : 'Não foi possível carregar o histórico de sincronização.');
            setLogs([]);
        } finally {
            setLoading(false);
        }
    ...
    const filteredLogs = logs.filter((l) =>
        (ENTITY_LABEL[l.entityType] || l.entityType).toLowerCase().includes(searchTerm.toLowerCase()) ||
                ...
                <CardContent>
                    ...
                    {loading ? (
                        ...
                        <EmptyState
                            ...
                            title={logs.length === 0 ? 'Nenhuma sincronização registrada ainda' : 'Nenhum resultado para este filtro'}
                            description={
                                logs.length === 0
                                ...
                                <tbody>
                                    {filteredLogs.map((log) => (
                                        <tr key={log.id} className="border-b border-line/50 hover:bg-surface-2/40 transition-colors">
                                            ...
                                            <td className="py-2.5 px-3 font-bold text-ink">{ENTITY_LABEL[log.entityType] || log.entityType}</td>
                                            <td className="py-2.5 px-3 font-mono text-ink-2">{log.bitrixRecordId || '—'}</td>
                                            <td className="py-2.5 px-3">
                    ...
                    <Card className="w-full max-w-xl shadow-2xl border-line">
                        <CardHeader className="flex flex-row items-center justify-between border-b border-line pb-4">
                            <div>
                                ...
                                <CardDescription>
                                    {selectedLog.direction === 'inbound' ? 'Bitrix → Atlas' : 'Atlas → Bitrix'} — {ENTITY_LABEL[selectedLog.entityType] || selectedLog.entityType}
                        ...
                        <CardContent className="pt-4 space-y-3 text-xs">
                            <div className="flex items-center gap-2">
                                <Badge variant={statusBadgeVariant(selectedLog.status)}>{statusLabel(selectedLog.status)}</Badge>
                                <span className="text-ink-2">{new Date(selectedLog.createdAt).toLocaleString('pt-BR')}</span>
                            </div>

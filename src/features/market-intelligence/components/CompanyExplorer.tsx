        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/35" role="dialog" aria-modal="true" aria-labelledby="company-detail-title">
            ...
            <aside className="relative h-full w-full max-w-2xl overflow-y-auto bg-[#F7F7F5] p-5 shadow-2xl md:p-7">
                <div className="flex items-start justify-between gap-4">
                    <div><p className="text-[10px] font-black uppercase tracking-[.18em] text-[#FF5618]">Empresa observada</p>
                    ...
                    <button type="button" onClick={onClose} className="rounded-xl border border-slate-200 bg-white p-2 text-slate-600 focus-visible:ring-2 focus-visible:ring-[#FF5618]"><X className="h-5 w-5" /></button>
                </div>
    ...
    return (
        <section className="space-y-4" aria-labelledby="company-explorer-title">
            <div className="rounded-3xl border border-slate-200 bg-white p-5 md:p-7">
                <div className="flex items-start gap-3"><div className="rounded-2xl bg-orange-50 p-2.5 text-[#FF5618]"><Building2 className="h-5 w-5" /></div><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-[#FF5618]">Base empresarial detalhada</p><h2 id="company-explorer-title" className="text-2xl font-black text-[#333333]">Empresas reais por território</h2><p className="mt-1 text-sm text-slate-500">Filtros e paginação são executados no servidor sobre o snapshot publicado.</p></div></div>
                <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                    <label className="text-xs font-bold text-slate-600">UF<input value={draft.uf ?? ''} maxLength={2} onChange={(event) => setDraft((current) => ({ ...current, uf: event.target.value.toUpperCase(), municipioIbge: '' }))} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#FF5618]" /></label>
                    <label className="text-xs font-bold text-slate-600">Município<select value={draft.municipioIbge ?? ''} onChange={(event) => setDraft((current) => ({ ...current, municipioIbge: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#FF5618]"><option value="">Todos</option>{territories.map((item) => <option key={item.municipioIbge} value={item.municipioIbge}>{item.municipioNome} ({number.format(item.activeCompanies)} ativas)</option>)}</select></label>
                    <label className="text-xs font-bold text-slate-600">CNPJ<input value={draft.cnpj ?? ''} onChange={(event) => setDraft((current) => ({ ...current, cnpj: event.target.value }))} placeholder="00.000.000/0000-00" className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#FF5618]" /></label>
                    <label className="text-xs font-bold text-slate-600">Razão social<input value={draft.razaoSocial ?? ''} onChange={(event) => setDraft((current) => ({ ...current, razaoSocial: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#FF5618]" /></label>
                    <label className="text-xs font-bold text-slate-600">Nome fantasia<input value={draft.nomeFantasia ?? ''} onChange={(event) => setDraft((current) => ({ ...current, nomeFantasia: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#FF5618]" /></label>
                    <label className="text-xs font-bold text-slate-600">CNAE<input value={draft.cnae ?? ''} onChange={(event) => setDraft((current) => ({ ...current, cnae: event.target.value }))} placeholder="49.30-2-02" className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#FF5618]" /></label>
            ...
            {loading && <div className="flex min-h-48 items-center justify-center rounded-3xl border border-slate-200 bg-white"><Loader2 className="mr-2 h-5 w-5 animate-spin text-[#FF5618]" />Carregando snapshot empresarial…</div>}
            ...
            {detailLoading && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35"><div className="rounded-2xl bg-white p-5 text-sm font-bold"><Loader2 className="mr-2 inline h-5 w-5 animate-spin text-[#FF5618]" />Carregando empresa…</div></div>}

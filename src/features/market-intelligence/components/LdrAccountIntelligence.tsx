...
function CapabilityCard({ title, capability, icon: Icon }: { title: string; capability: Capability; icon: typeof Zap }) {
  ...
  return (
    <article className={`rounded-2xl border p-4 ${tone}`}>
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2"><Icon className="h-4 w-4" /><h3 className="text-sm font-black">{title}</h3></div>
        <span className="text-[9px] font-black uppercase tracking-[0.15em]">{capability.status.replaceAll('_', ' ')}</span>
      </div>
...
export function LdrAccountIntelligence() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/companies/market-intelligence/${encodeURIComponent(cnpj)}/intelligence`, {
        credentials: 'include',
      });
  ...
  return (
    <div className="space-y-5 p-4 md:p-6 lg:p-8">
      ...
      {error && (
        <div role="alert" className="flex gap-3 rounded-3xl border border-rose-200 bg-rose-50 p-5 text-sm text-rose-800">
        ...
        <>
          <section className="grid gap-4 xl:grid-cols-[1.4fr_.6fr]">
            ...
            <article className="rounded-3xl border border-orange-200 bg-orange-50 p-5 md:p-6">
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-orange-700">Account Score</p>
          ...
          <section className="grid gap-3 md:grid-cols-3">
            ...
            <CapabilityCard title="Grupo econômico" capability={account.economicGroup} icon={Building2} />
          </section>
          ...
          <section className="grid gap-4 xl:grid-cols-[1fr_.8fr]">
            ...
            <article className={`rounded-3xl border p-5 md:p-6 ${account.nextBestAction.status === 'READY' ? 'border-emerald-200 bg-emerald-50 text-emerald-900' : 'border-amber-200 bg-amber-50 text-amber-900'}`}>
              ...
              {account.nextBestAction.reasons.length > 0 && <ul className="mt-4 space-y-2 text-xs">{account.nextBestAction.reasons.map((reason) => <li key={reason}>• {reason}</li>)}</ul>}
            </article>
          </section>

    ...
    return (
        <section className="rounded-3xl border border-line bg-surface p-5 md:p-7" aria-labelledby="economic-audit-title">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <span className="inline-flex items-center gap-2 rounded-full border border-line bg-surface-2 px-3 py-2 text-[11px] font-black text-ink-2"><History className="h-4 w-4" aria-hidden="true" />APPEND-ONLY</span>
            </div>
            ...
            <div className="mt-5 overflow-hidden rounded-2xl border border-line">
                <div className="flex items-center justify-between border-b border-line bg-surface-2 px-4 py-3">
                    <p className="text-xs font-black text-ink">Histórico recente</p>
                    <button type="button" aria-label="Atualizar histórico econômico" onClick={() => { setLoading(true); void refresh(); }} className="text-ink-2 hover:text-ink"><RotateCcw className="h-4 w-4" aria-hidden="true" /></button>
                </div>
                {loading ? (
                    <div className="flex items-center gap-2 p-4 text-xs font-bold text-ink-2"><Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" />Carregando snapshots...</div>
                ) : items.length === 0 ? (
                    <p className="p-4 text-xs text-ink-2">Nenhum cenário auditável registrado para esta organização.</p>
                ) : (
                            ...
                            <article key={item.id} className="grid gap-3 p-4 lg:grid-cols-[1fr_auto] lg:items-center">
                                <div>
                                    <div className="flex flex-wrap items-center gap-2">
                                        ...
                                        <span className="rounded-full bg-surface-2 px-2 py-1 text-[10px] font-black text-ink-2">{recommendationLabel(item.recommendation)}</span>
                                    ...
                                    <p className="mt-1 text-xs text-ink-2">{item.baseCity}/{item.uf} · {item.radiusKm} km · {item.commercialScenario} · {new Date(item.createdAt).toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' })}</p>
                                    <p className="mt-1 text-[11px] text-ink-2">Custo {money.format(item.monthlyFixedCost)} · Payback {item.paybackMonth === null ? 'N/A' : `${item.paybackMonth}m`} · ROI 12m {item.roi12Pct === null ? 'N/A' : `${pct.format(item.roi12Pct)}%`} · hash {item.snapshotHash.slice(0, 10)}</p>
                                </div>
                                <button type="button" aria-label={`Carregar snapshot ${item.label}`} disabled={loadingId === item.id} onClick={() => void load(item.id)} className="inline-flex items-center justify-center gap-2 rounded-xl border border-line bg-surface px-3 py-2 text-xs font-black text-ink hover:bg-surface-2 disabled:opacity-50">

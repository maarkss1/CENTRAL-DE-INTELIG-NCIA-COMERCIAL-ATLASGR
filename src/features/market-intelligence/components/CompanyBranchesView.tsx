...
export function CompanyBranchesView({ cnpj, companyName, onSelectBranch }: CompanyBranchesViewProps) {
        ...
        <div className="space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                    ...
                    <p className="text-xs text-ink-2">
                        CNPJ Raiz: <b className="font-mono text-ink">{cnpjRoot.replace(/^(\d{2})(\d{3})(\d{3})/, '$1.$2.$3')}</b> · {branches.length} unidade{branches.length !== 1 ? 's' : ''} encontrada{branches.length !== 1 ? 's' : ''}
                    </p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="p-8 rounded-3xl border border-line bg-surface/50 text-center space-y-2">
                    ...
                    <p className="text-xs font-semibold text-ink">Nenhuma outra filial encontrada com este CNPJ Raiz.</p>
                </div>
            ) : (
                    ...
                    {matriz && (
                        <div className="p-5 rounded-3xl border-2 border-brand/30 bg-brand/5 shadow-xs space-y-3">
                            <div className="flex flex-wrap items-start justify-between gap-3">
                                ...
                                <div className="flex items-center gap-2">
                                    <button
                                        ...
                                        className="px-3 py-1.5 rounded-xl bg-brand text-white text-xs font-bold hover:brightness-110 flex items-center gap-1 shadow-sm transition-all disabled:opacity-50"
                                    >
                                        {approvingCnpj === matriz.cnpj ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />}
                                        Aprovar Matriz
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-wrap items-center gap-4 text-xs pt-2 border-t border-line/50 text-ink-2">
                                ...
                                <span>CNAE: <b>{matriz.cnaePrincipal}</b></span>
                                <span>Porte: <b>{matriz.porte || 'N/A'}</b></span>
                                <span>Situação: <b className={matriz.situacaoCadastral === 'ATIVA' ? 'text-emerald-600' : 'text-amber-600'}>{matriz.situacaoCadastral}</b></span>
                            </div>
                        </div>
                    )}
                                ...
                                {filiais.map((f) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-2 text-xs text-ink-2">
                                            ...
                                            <span>{f.municipioNome || 'Município'}/{f.uf || 'UF'}</span>
                                        </div>

                                        <div className="flex items-center justify-between pt-2 border-t border-line/50 text-xs">
                                            <span className={f.situacaoCadastral === 'ATIVA' ? 'text-emerald-600 font-bold' : 'text-amber-600 font-bold'}>

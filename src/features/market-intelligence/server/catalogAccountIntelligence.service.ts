...
/**
 * Inteligência de conta derivada só do catálogo global de Market Intelligence (dataset CNPJ
 * público) — nunca do CRM/tenant (essa é `accountIntelligence.service.ts`/`accountIntelligence.routes.ts`,
 * com organizationId/RLS reais). Duas features distintas do LDR que chegaram a colidir no mesmo
 * nome de arquivo em branches diferentes; separadas aqui para as duas compilarem sem conflito —
 * ver `.agents/runs/onda-26.md` para o histórico da reconciliação.
 */

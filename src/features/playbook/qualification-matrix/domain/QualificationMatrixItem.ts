...
export type QualificationCategory = 'Situação' | 'Problema' | 'Implicação/Custo' | 'Necessidade/ROI';
...
export interface QualificationMatrixItemRepository extends Repository<QualificationMatrixItem> {
    findAllWithFilters(organizationId: string, brand?: string, page?: number, limit?: number): Promise<{ data: QualificationMatrixItem[]; meta: unknown }>;
}

import { QualificationMatrixItem, QualificationMatrixItemRepository } from '../domain/QualificationMatrixItem';
import { prisma } from '../../../../lib/prisma';
import { Prisma } from '@prisma/client';

export class PrismaQualificationMatrixRepository implements QualificationMatrixItemRepository {
    async findAllWithFilters(organizationId: string, brand?: string, page: number = 1, limit: number = 200): Promise<{ data: QualificationMatrixItem[]; meta: unknown }> {
        ...
        const [data, total] = await prisma.$transaction([
            prisma.qualificationMatrixItem.findMany({ where, skip, take: limit, orderBy: { createdAt: 'asc' } }),
    ...
    async create(organizationId: string, data: Partial<QualificationMatrixItem>): Promise<QualificationMatrixItem> {
    ...
    async update(organizationId: string, id: string, data: Partial<QualificationMatrixItem>): Promise<QualificationMatrixItem> {
        const existing = await prisma.qualificationMatrixItem.findFirst({ where: { id, organizationId } });
        if (!existing) throw new Error('Item de qualificação não encontrado');
        const updated = await prisma.qualificationMatrixItem.update({ where: { id }, data: data as Prisma.QualificationMatrixItemUncheckedUpdateInput });

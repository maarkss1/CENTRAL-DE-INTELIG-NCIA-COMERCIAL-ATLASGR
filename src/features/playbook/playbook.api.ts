...
export const playbookApi = {
    ...
    listQualifications: (brand?: string) =>
        api.get<{ data: QualificationMatrixItem[]; meta: unknown }>(`/api/playbook/qualification-matrix${brand ? `?brand=${encodeURIComponent(brand)}` : ''}`)
            .then((res) => res.data),
    ...
    listObjections: (brand?: string) =>
        api.get<{ data: ObjectionMatrixItem[]; meta: unknown }>(`/api/playbook/objection-matrix${brand ? `?brand=${encodeURIComponent(brand)}` : ''}`)
            .then((res) => res.data),

...
export class PrismaObjectionMatrixRepository implements ObjectionMatrixItemRepository {
    async findAllWithFilters(organizationId: string, brand?: string, page: number = 1, limit: number = 200): Promise<{ data: ObjectionMatrixItem[]; meta: unknown }> {
        ...
        const [data, total] = await prisma.$transaction([
            prisma.objectionMatrixItem.findMany({ where, skip, take: limit, orderBy: { createdAt: 'asc' } }),
    ...
    async create(organizationId: string, data: Partial<ObjectionMatrixItem>): Promise<ObjectionMatrixItem> {
    ...
    async update(organizationId: string, id: string, data: Partial<ObjectionMatrixItem>): Promise<ObjectionMatrixItem> {
        ...
        const updated = await prisma.objectionMatrixItem.update({ where: { id }, data: data as Prisma.ObjectionMatrixItemUncheckedUpdateInput });

...
export async function runAgentMemoryCleanupSweep(): Promise<AgentMemoryCleanupResult> {
    ...
    for (const org of organizations) {
        try {
            ...
            logger.error({ err, organizationId: org.id }, 'Falha ao expurgar AgentMemory desta organização');
        }
    }
...
export function createAgentMemoryCleanupWorker() {
    const worker = new Worker(AGENT_MEMORY_CLEANUP_QUEUE_NAME, async (_job) => runAgentMemoryCleanupSweep(), {
    ...
    worker.on('error', (err) => {
        logger.warn({ message: err.message }, 'AgentMemoryCleanup worker error suppressed (Redis offline)');
    });

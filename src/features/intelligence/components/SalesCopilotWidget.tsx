  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 w-14 h-14 bg-gradient-to-tr from-brand to-orange-500 rounded-full flex items-center justify-center shadow-xl hover:scale-110 transition-transform z-50 ${isOpen ? 'hidden' : 'flex'}`}
      >
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {/* Header */}
            <div className="bg-gradient-to-r from-brand to-orange-500 p-4 flex items-center justify-between">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.map((msg) => (
                <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${msg.role === 'user' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600' : 'bg-orange-100 dark:bg-orange-900/30 text-brand'}`}>
            ...
            <div className="p-4 bg-gray-50 dark:bg-black/20 border-t border-gray-200 dark:border-white/10">
              ...
              >
                ...
                <button
                  ...
                  className="w-10 h-10 bg-brand hover:bg-orange-600 disabled:opacity-50 text-white rounded-xl flex items-center justify-center transition-colors"
                >

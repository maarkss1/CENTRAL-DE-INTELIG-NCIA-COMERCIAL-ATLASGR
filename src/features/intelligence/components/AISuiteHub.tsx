...
const CAPABILITIES: AICapability[] = [
    {
        ...
        description: 'Pesquisa notícias recentes na web e cria ganchos ultra-personalizados de 2 frases conectando com dores de logística.',
    ...
    },
    {
        ...
        description: 'Gera roteiros de abordagem consultiva adaptados ao perfil do decisor e porte da frota.',
        modelDefault: 'llama3.1:8b',
        samplePayload: { companyName: 'Expresso Logística', contactName: 'Marcos Silveira', channel: 'call', stepNumber: 1, valueProposition: 'Redução de sinistros com telemetria 4G' },
    },
    {
        id: 3,
    ...
    },
    {
        ...
        description: 'Worker que sintetiza o funil de vendas, métricas de conversão e gargalos operacionais.',
        modelDefault: 'llama3.1:8b',
        samplePayload: {},
    },
    {
        ...
        description: 'Classifica a pontuação do lead (A/B/C/D) com base em frota estimada, CNAE e faturamento.',
        modelDefault: 'deepseek-r1:8b',
        samplePayload: { companyName: 'LOGISTICA & TRANSPORTE VALE VERDE LTDA', jobTitle: 'coord operacoes frota', segmentHint: 'Transporte de Químicos e Perigosos' },
    },
    {
        id: 6,
    ...
    },
    {
        ...
        icon: RefreshCw,
        endpoint: '/api/intelligence/suite/bitrix-hygiene',
        description: 'Higieniza dados brutos, remove sufixos desnecessários (LTDA/ME) e padroniza cargos confusos.',
    ...
    },
    {
        ...
        icon: Layers,
        endpoint: '/api/intelligence/suite/cadence-step',
        description: 'Reescreve e-mails e mensagens de acordo com a reação anterior do lead (abriu, sumiu, pediu tempo).',
        modelDefault: 'llama3.1:8b',
        samplePayload: {
    ...
    },
    {
        ...
        description: 'Simula um cliente difícil com objeções reais para treinar quebra de objeção antes da ligação real.',
        modelDefault: 'llama3.1:8b',
        samplePayload: {
            persona: {
            ...
            history: [],
            userMessage: 'Olá Sr. Valdir, tudo bem? Sei que sua frota já tem rastreamento, mas nossa tecnologia reduz os falsos alertas em 80%.',
        },
    },
    {
        id: 10,
        ...
        samplePayload: {
            ...
            fleetSize: 45,
            diagnosedPains: ['Roubo frequente em trechos urbanos', 'Alto custo com horas extras de motoristas'],
            proposedModules: ['Telemetria Avançada 4G', 'Trava Eletromagnética de Baú', 'Sensor de Fadiga'],
    ...
    },
    {
        ...
        description: 'Extrai a próxima tarefa com data limite e mensagem sugerida logo após o vendedor registrar uma anotação.',
        modelDefault: 'llama3.1:8b',
        samplePayload: {
            leadOrClientName: 'TransDelta',
            stage: 'Demonstração Realizada',
            rawNote: 'Reunião excelente com o Diretor Cláudio. Ele gostou do painel de telemetria, mas pediu para enviar até sexta a cotação com desconto para pagamento à vista da instalação.',
    ...
    },
    {
        ...
        icon: TrendingUp,
        endpoint: '/api/intelligence/win-loss-analysis',
        description: 'Identifica padrões estatísticos e comportamentais em negócios ganhos vs. perdidos no CRM.',
        modelDefault: 'deepseek-r1:8b',
        samplePayload: {},
    },
    {
        ...
        description: 'Analisa tickets, quedas de telemetria e atrasos financeiros para alertar risco de cancelamento.',
        modelDefault: 'deepseek-r1:8b',
        samplePayload: {
    ...
    },
    {
        ...
        description: 'Analisa segmento e urgência do lead para direcionar ao vendedor com maior taxa histórica de fechamento.',
        modelDefault: 'deepseek-r1:8b',
        samplePayload: {
            lead: {
                leadId: 'lead_992',
            ...
            reps: [
                { repId: 'rep_1', name: 'Rodrigo Matos', specialties: ['Frota Pesada', 'Químicos', 'PGR'], winRatePercent: 42, currentLeadCount: 8 },
                { repId: 'rep_2', name: 'Juliana Paes', specialties: ['Distribuição Urbana', 'Frotas Leves'], winRatePercent: 35, currentLeadCount: 4 },
            ],
        },
    },
    {
    ...
    },
    {
        ...
        description: 'Converte transcrições brutas de chamadas em atas executivas com pontos acordados e tarefas atribuídas.',
        modelDefault: 'llama3.1:8b',
        samplePayload: {
            ...
            rawTranscript: 'Leonardo: Demonstramos a trava de quinta roda e o bloqueador anti-jammer. Mauro: Gostei muito da redundância via satélite. Vamos avançar com o teste em 10 veículos na próxima terça.',
        },
    },
    {
        id: 17,
        ...
        description: 'Classifica a severidade de alertas (P0 a P3) e entrega o POP imediato de contenção de sinistro.',
        modelDefault: 'llama3.1:8b',
        samplePayload: {
            alertId: 'ALT-88912',
            ...
            telemetryDataSummary: 'Veículo desviou 15km da rodovia BR-116 em trecho classificado como Zona Vermelha. Sensor de baú acionado com ignição ligada.',
    ...
    },
    {
        ...
        description: 'Gera feedback personalizado e micro-hábitos semanais com base nos KPIs individuais de cada vendedor.',
        modelDefault: 'llama3.1:8b',
        samplePayload: {
    ...
    },
    {
        ...
        description: 'Cria capítulos completos de playbook com perguntas SPIN e roteiros de contorno de objeções.',
        modelDefault: 'llama3.1:8b',
        samplePayload: {
            topic: 'Como Superar a Objeção "Já tenho seguro e a seguradora não exige telemetria avançada"',
    ...
    },
    {
        ...
        description: 'Detecta e mascara CPFs, placas, dados bancários e dados pessoais sensíveis com score de conformidade.',
        modelDefault: 'qwen2.5:7b',
        samplePayload: {
            rawText: 'O motorista Carlos Alberto Silva (CPF 123.456.789-10, CNH 987654321) informou que o frete da placa BRA2E19 foi pago na conta corrente 1234-5 do Banco do Brasil.',
...
export function AISuiteHub() {
    ...
    const [payloadText, setPayloadText] = useState<string>(JSON.stringify(CAPABILITIES[0].samplePayload, null, 2));
    ...
    const handleRunCapability = async () => {
        ...
        try {
            ...
            const res = await api.post<{ success: boolean; data?: unknown; result?: unknown }>(selectedCap.endpoint, parsedBody);
            ...
            <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-border/40 pb-5">
                <div>
                    ...
                    <p className="text-sm text-ink-2 mt-0.5">
                        Execute, teste e orquestre os 20 motores de IA integrados à operação da AtlasGR e TotalTrac.
                    </p>
                </div>
                    ...
                    <Card className="border border-border/60 shadow-sm bg-surface">
                        <div className="p-5 border-b border-border/40 flex items-center justify-between gap-4">
                            <div className="flex items-center gap-3">
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${accent.bgSoft} ${accent.text}`}>
                                    <selectedCap.icon className="w-5 h-5" />
                                </div>
                                <div>
                                    <div className="flex items-center gap-2">
                                        <span className="text-xs font-mono font-bold text-ink-3">Motor #{selectedCap.id}</span>
                                        <CardTitle className="text-base font-bold text-ink">{selectedCap.title}</CardTitle>
                                    </div>
                                    <CardDescription className="text-xs text-ink-2 mt-0.5">
                                        Endpoint: <code className="font-mono text-primary bg-primary/10 px-1 py-0.5 rounded text-[11px]">{selectedCap.endpoint}</code>
                                    </CardDescription>
                                </div>
                            </div>
                                ...
                                <AnimatePresence mode="wait">
                                    ...
                                    {!executionResult && !executionError && (
                                        <div className="p-8 rounded-lg border border-dashed border-border/60 text-center text-ink-3 text-xs flex flex-col items-center justify-center gap-2">
                                            ...
                                            <span>Clique em &ldquo;Executar Motor&rdquo; acima para processar com o modelo configurado no servidor.</span>
                                        </div>
                                    )}
                                </AnimatePresence>
                            </div>

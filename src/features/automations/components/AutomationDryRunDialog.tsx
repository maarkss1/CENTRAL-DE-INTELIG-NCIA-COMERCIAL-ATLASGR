...
import { FlaskConical, Loader2, AlertTriangle, CheckCircle2, CircleSlash, RefreshCw } from 'lucide-react';
...
function describeOutcome(record: DryRunRecord): string {
    ...
    if (action === 'Notificar equipe') {
        const channel = details.channel === 'email' ? `e-mail para ${String(details.to ?? '?')}` : 'aviso interno';
        return `${channel}: "${String(details.title ?? '')}"`;
    }
    if (action === 'Criar atividade') {
        const due = details.dueDate ? new Date(String(details.dueDate)).toLocaleDateString('pt-BR') : '?';
        return `${String(details.type ?? 'Follow_up')} para ${String(details.owner ?? '—')}, prazo ${due}`;
    }
    if (action === 'Ligar via SDR de Voz') {
        return details.targetNumber ? `Ligar para ${String(details.targetNumber)}` : 'Ligação de SDR de voz';
    }
    return action;
}
...
function ResultRow({ record }: { record: DryRunRecord }) {
    ...
    return (
        <li className="flex items-start gap-3 px-3 py-2.5 border-b border-line last:border-b-0">
            {wouldFire ? (
                <CheckCircle2 className="w-4 h-4 mt-0.5 shrink-0 text-ok-active dark:text-ok" aria-hidden="true" />
            ) : (
...
export function AutomationDryRunDialog({ automation, onClose }: AutomationDryRunDialogProps) {
    ...
    const [result, setResult] = useState<Awaited<ReturnType<typeof automationsApi.dryRun>> | null>(null);
            ...
            {!loading && !error && result && (
                <div className="space-y-4">
                    <div className="grid grid-cols-3 gap-2">
                        ...
                        <div className="rounded-xl border border-line bg-surface-2 px-3 py-2 text-center">
                            <p className="text-lg font-bold text-ok-active dark:text-ok">{result.wouldFireCount}</p>
                    ...
                    <div className="flex justify-end">
                        <Button variant="outline" size="sm" onClick={() => automation && void run(automation.id)}>

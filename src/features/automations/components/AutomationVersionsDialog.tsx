...
import { automationsApi, type Automation, type AutomationVersionTimelineEntry } from '../automations.api';
...
export function AutomationVersionsDialog({ automation, onClose }: AutomationVersionsDialogProps) {
    ...
    const [timeline, setTimeline] = useState<Awaited<ReturnType<typeof automationsApi.versions>> | null>(null);
            ...
            {!loading && !error && timeline && (
                <div className="space-y-4">
                    <div className="rounded-xl border border-line bg-surface-2 px-3 py-2">
                        <p className="text-[11px] uppercase tracking-wide text-ink-2 font-semibold mb-0.5">Estado atual</p>
                    ...
                    {timeline.history.length === 0 ? (
                        <div className="text-center py-8 border border-dashed border-line rounded-xl">
                            ...
                            <p className="text-sm text-ink-2">Esta automação ainda não foi editada desde a criação.</p>
                        </div>
                    ) : (
                        <ol>

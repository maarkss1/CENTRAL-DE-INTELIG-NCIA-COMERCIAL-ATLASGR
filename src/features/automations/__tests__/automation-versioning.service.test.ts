...
describe('diffAutomationSnapshots', () => {
    ...
    it('detecta múltiplos campos alterados ao mesmo tempo', () => {
        const diff = diffAutomationSnapshots(base, { ...base, action: 'Criar atividade', actionConfig: { dueInDays: 2 } });
...
describe('automationVersioningService.buildTimeline', () => {
    ...
    it('uma edição: histórico com 1 entrada, diff contra o estado atual', async () => {
        ...
        expect(entry.diffToNext).toEqual([{ field: 'Nome', before: 'Nome antigo', after: 'Nome novo' }]);
    });
    ...
    it('remoção registra changeReason "delete"', async () => {
        ...
        const timeline = await automationVersioningService.buildTimeline('org-1', makeAutomation({ id: automationId }));
    ...
    it('isola por organizationId — nunca mistura histórico de outro tenant', async () => {
        ...
        const timelineOrgB = await automationVersioningService.buildTimeline('org-B', makeAutomation({ id: automationId }));

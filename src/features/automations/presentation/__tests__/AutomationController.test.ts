...
describe('AutomationController', () => {
    ...
    describe('updateAutomation', () => {
        ...
        it('responde 200 com o registro atualizado, passando o actor autenticado', async () => {
            ...
            await controller.updateAutomation(buildRequest({ params: { id: 'auto-1' }, body: { enabled: false } }), res, next);

            expect(useCases.updateAutomation).toHaveBeenCalledWith('org-1', 'auto-1', { enabled: false }, { userId: 'user-1', email: 'user@atlasgr.com.br' });
            expect(res.json).toHaveBeenCalledWith({ success: true, data: { id: 'auto-1', enabled: false } });
        });
    });
    ...
    describe('dryRunAutomation', () => {
        ...
        it('extrai limit do body quando presente e ignora query', async () => {
            ...
            await controller.dryRunAutomation(buildRequest({ params: { id: 'auto-1' }, body: { limit: 10 }, query: { limit: '99' } }), res, next);

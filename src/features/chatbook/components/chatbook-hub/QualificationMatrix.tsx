...
export function QualificationMatrix({ activeBrand, brandInfo }: { activeBrand: Brand; brandInfo: BrandInfo }) {
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            <div className="bg-surface/80 backdrop-blur-xl rounded-[3rem] p-8 md:p-12 border border-line shadow-[0_20px_40px_rgba(0,0,0,0.03)]">
                ...
                <p className="text-ink-2 text-base font-medium mb-10 max-w-3xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {QUALIFICATION_CRITERIA.map((crit, idx) => (
                        <motion.div whileHover={{ y: -5 }} key={idx} className="bg-surface-2/60 p-8 rounded-[2rem] border border-line shadow-[0_10px_30px_rgba(0,0,0,0.03)] space-y-6 flex flex-col justify-between">
                            <div className="flex items-start justify-between gap-4">
                                <h3 className="font-black text-xl text-ink tracking-tight">{crit.category}</h3>
                            ...
                            <div>
                                <p className="text-[10px] font-black uppercase tracking-widest text-ink-2 mb-2">Critério para {brandInfo.name}</p>
                            ...
                            <div className={`p-5 rounded-[1.5rem] border ${activeBrand === 'totaltrac' ? 'bg-sky-50/50 border-sky-100' : 'bg-orange-50/50 border-orange-100'}`}>
                                ...
                                <p className="text-sm text-ink italic font-medium leading-relaxed">

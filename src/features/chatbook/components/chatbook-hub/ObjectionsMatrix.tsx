            ...
            <div className="bg-surface/80 backdrop-blur-xl rounded-[3rem] p-8 md:p-12 border border-line shadow-[0_20px_40px_rgba(0,0,0,0.03)]">
                ...
                <div className="space-y-6">
                    {OBJECTIONS_DATA.map((obj) => (
                        <motion.div whileHover={{ scale: 1.01 }} key={obj.id} className="p-8 rounded-[2rem] border border-line bg-surface-2/60 shadow-[0_10px_30px_rgba(0,0,0,0.03)] hover:shadow-lg transition-all duration-300 space-y-5">
                            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 border-b border-line pb-5">
                                <div>
                                    <h3 className="font-black text-xl text-ink tracking-tight">{obj.title}</h3>
                                    <p className="text-sm text-ink-2 font-medium mt-1">{obj.description}</p>
                                </div>
                            ...
                            <div className={`p-5 rounded-[1.5rem] border ${activeBrand === 'totaltrac' ? 'bg-sky-50/50 border-sky-100' : 'bg-orange-50/50 border-orange-100'}`}>
                                ...
                                <div className="text-base text-ink leading-relaxed font-medium italic">

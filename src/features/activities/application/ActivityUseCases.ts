...
export class ActivityUseCases {
    ...
    async snoozeActivity(organizationId: string, id: string, duration: '2h' | 'tomorrow' | 'next_week') {
        const now = new Date();
    ...
    async generateIcalFeed(organizationId: string, owner?: string): Promise<string> {
        ...
        for (const act of filtered) {
            ...
            const leadObj = (act as { lead?: { company?: { tradeName?: string; legalName?: string }; contact?: { name?: string }; title?: string } }).lead;

...
export function ActiveCallView({
  ...
  return (
    <motion.div
      ...
      className="bg-[#0b0f19] text-white rounded-[3rem] border border-line shadow-[0_20px_40px_rgba(0,0,0,0.05)] overflow-hidden flex flex-col h-[600px] relative"
    >
      <div
        className={`absolute inset-0 opacity-20 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] ${activeBrand === 'totaltrac' ? 'from-sky-500 via-[#0b0f19] to-[#0b0f19]' : 'from-orange-500 via-[#0b0f19] to-[#0b0f19]'}`}
      />
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative z-10 space-y-12">
        ...
        <div className="relative">
          {botSpeaking && (
            <>
              <motion.div
                ...
                className={`absolute inset-0 rounded-full opacity-30 blur-md ${activeBrand === 'totaltrac' ? 'bg-sky-500' : 'bg-orange-500'}`}
              />
              <motion.div
                animate={{ scale: [1, 1.4, 1] }}
                transition={{ repeat: Infinity, duration: 1.5, delay: 0.2 }}
                className={`absolute inset-0 rounded-full opacity-10 blur-xl ${activeBrand === 'totaltrac' ? 'bg-sky-500' : 'bg-orange-500'}`}
              />
            </>
          )}
          ...
          <div className="w-32 h-32 rounded-full bg-gray-800 border-4 border-gray-700 flex items-center justify-center relative z-10 shadow-2xl">
            <Bot
              className={`w-16 h-16 ${botSpeaking ? (activeBrand === 'totaltrac' ? 'text-sky-400' : 'text-orange-400') : 'text-gray-400'}`}
            />
          </div>
        </div>
        ...
        <div className="h-24 px-8 w-full max-w-2xl flex flex-col items-center justify-center text-center">
          ...
          {botSpeaking && (
            <p
              className={`text-lg font-medium leading-relaxed ${activeBrand === 'totaltrac' ? 'text-sky-100' : 'text-orange-100'}`}
            >

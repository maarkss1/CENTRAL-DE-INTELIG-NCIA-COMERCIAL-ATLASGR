...
const STATUS_OPTIONS: DocumentStatus[] = ['Rascunho', 'Enviado', 'Visualizado', 'Aceito', 'Recusado', 'Vencido', 'Pago', 'Cancelado'];
...
export function PropostasList() {
    ...
    const filtered = useMemo(() => {
        ...
        return documents.filter((doc) => {
            ...
            if (term && !doc.number.toLowerCase().includes(term) && !doc.title.toLowerCase().includes(term)) return false;
            return true;
        });
    }, [documents, searchTerm, statusFilter, typeFilter]);

    const selectedDocument = selectedDocumentId ? documents.find((d) => d.id === selectedDocumentId) ?? null : null;
            ...
            <div className="flex-1 overflow-y-auto bg-bg text-ink p-6 md:p-8">
                <div className="max-w-6xl mx-auto">
                    <PropostaDetail
                        ...
                        onEdit={() => { setEditingDocument(selectedDocument); setIsFormOpen(true); }}
                        onChanged={load}
                    />
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto space-y-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-surface p-6 rounded-3xl border border-line">
                    <div>
                        ...
                        <p className="text-ink-2 text-sm mt-1">Orçamentos, propostas, faturas e contratos — com versionamento e assinatura eletrônica.</p>
                    </div>
                    <button
                        onClick={() => { setEditingDocument(null); setIsFormOpen(true); }}
                    ...
                    <div className="bg-surface border border-line rounded-3xl p-8">
                        {documents.length === 0 ? (
                            <EmptyState
                                ...
                                onAction={() => { setEditingDocument(null); setIsFormOpen(true); }}
                                ...
                                <tbody>
                                    {filtered.map((doc) => (
                                        ...
                                        >
                                            ...
                                            <td className="p-4 text-ink-2">{doc.company?.tradeName || doc.lead?.title || '—'}</td>
                                            <td className="p-4">
                                                <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold border ${STATUS_STYLES[doc.status]}`}>
                                                    {doc.status}
                                                </span>
                                            </td>
                                            <td className="p-4 text-right font-bold text-ink">{money.format(doc.total)}</td>
                                            <td className="p-4 text-right text-ink-2">{new Date(doc.issueDate).toLocaleDateString('pt-BR')}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>

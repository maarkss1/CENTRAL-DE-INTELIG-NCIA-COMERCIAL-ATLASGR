...
function computeItemTotal(item: { quantity: number; unitPrice: number; discountPercent: number; taxPercent: number }): number {
...
export function PropostaForm({ document, onClose, onSave }: PropostaFormProps) {
    ...
    useEffect(() => {
        crm360Api.listProducts().then(setProducts).catch((err) => {
    ...
    useEffect(() => {
        reset(document
            ? {
                ...
                lineItems: document.lineItems.length > 0
    ...
    const total = Math.max(0, (watchedItems || []).reduce((sum, item) => sum + computeItemTotal({
            ...
            <form id="proposta-form" onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="space-y-2">
                        ...
                        {isEdit && <p className="text-xs text-ink-2">Tipo não pode ser alterado após a criação.</p>}
                    </div>
                            ...
                            <div key={field.id} className="rounded-xl border border-line bg-surface-2 p-3 space-y-2">
                                <div className="grid grid-cols-1 md:grid-cols-12 gap-2 items-start">
                                    ...
                                    <div className="md:col-span-1">
                                        <Input type="number" step="1" min="1" placeholder="Qtd" {...register(`lineItems.${index}.quantity`)} />
                                    </div>
                                    <div className="md:col-span-2">
                                        <Input type="number" step="0.01" min="0" placeholder="Preço unit." {...register(`lineItems.${index}.unitPrice`)} />
                                    </div>
                                    <div className="md:col-span-1">
                                        <Input type="number" step="1" min="0" max="100" placeholder="Desc %" {...register(`lineItems.${index}.discountPercent`)} />
                                    </div>
                                    <div className="md:col-span-1">
                                        <button
                                            type="button"
                ...
                <div className="flex justify-end gap-6 text-sm">
                    <span className="text-ink-2">Subtotal: <strong className="text-ink">{money.format(subtotal)}</strong></span>
                    <span className="text-ink-2">Total estimado: <strong className="text-brand">{money.format(total)}</strong></span>
                </div>
                ...
                {isEdit && (
                    <div className="space-y-2">
                        <Label htmlFor="doc-changeReason">Motivo da alteração (fica registrado no histórico de versões)</Label>

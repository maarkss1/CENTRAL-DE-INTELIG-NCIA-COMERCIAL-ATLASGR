...
const OPEN_LEAD_STATUSES = ['Lead_Recebido', 'Cadencia_Iniciada', 'Qualificacao_SDR', 'Reuniao_Agendada'] as const;
...
function leadTitle(lead: QueueLead): string {
    return lead.company?.tradeName || lead.company?.legalName || lead.contact?.name || `Lead ${lead.id}`;
}
...
async function resolveScope(
    ...
    connectionId: string,
): Promise<{ ownerName: string | undefined; assignedById: string | undefined; restricted: boolean; matched: boolean }> {
    ...
    return { ownerName: user?.name, assignedById, restricted: true, matched: !!user?.name && !!assignedById };
}

router.get('/queue', mesaRoles, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        const { organizationId } = (req as AuthRequest).user;
        const connection = await prisma.bitrixConnection.findFirst({ where: { organizationId }, orderBy: { createdAt: 'asc' } });
        if (!connection) {
            res.json({ success: true, data: { queue: [], current: null, connectionId: null, leadStatuses: [] } });
            return;
        }
        ...
        if (scope.restricted && !scope.matched) {
            res.json({
                ...
                meta: { warning: 'Seu usuário não foi encontrado no Bitrix24 (confira o e-mail cadastrado) — não é possível montar sua fila.' },
            });
            return;
        }
        ...
        const { ids } = await findUnimportedBitrixLeadIds(organizationId, connection.id, 50, { assignedById: scope.assignedById });
        if (ids.length > 0) {
...
router.post('/lead/:id/register', mesaRoles, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        const { organizationId } = (req as AuthRequest).user;
        const { id } = req.params;
        const body = req.body as {
            outcome?: 'contato' | 'reuniao_agendada' | 'reuniao_realizada' | 'retorno' | 'convertido' | 'sem_fit' | 'dados_invalidos' | 'revisao';
            note?: string;
        ...
        if (!body.note?.trim()) throw new AppError('Registre uma observação antes de continuar.', 400);

        const isDisqualify = body.outcome === 'sem_fit' || body.outcome === 'dados_invalidos';
        if (isDisqualify && !body.lossReasonId) throw new AppError('Selecione o motivo de desqualificação.', 400);

        const lead = await prisma.lead.findFirst({ where: { id, organizationId }, select: { id: true, owner: true, bitrixLeadId: true } });
        ...
        if (!hasRequiredRole(role, ['ADMIN', 'GESTOR'])) {
            const user = await prisma.user.findUnique({ where: { id: userId }, select: { name: true } });
            if (!user?.name || lead.owner !== user.name) throw new AppError('Este Lead não é seu.', 403);
        }
        ...
        await exportLeadToBitrixNow(organizationId, id, connection.id, body.bitrixStatusId ? { statusId: body.bitrixStatusId } : undefined);

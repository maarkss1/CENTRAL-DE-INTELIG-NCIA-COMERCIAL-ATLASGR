...
import { computeHealthScore, HEALTH_PILLAR_ORDER, HEALTH_SCORE_RULES } from '../application/healthScore';
import type {
    AgingReport,
    CrmQualityIndex,
    ExecutiveOverview,
...
function baseOverview(overrides: Partial<ExecutiveOverview> = {}): ExecutiveOverview {
    return {
        period: PERIOD,
        goal: { period: PERIOD, metric: 'NEW_MRR', amount: 100_000, currency: 'BRL', updatedAt: NOW.toISOString(), createdBy: 'user-1' },
        closedAmount: 0,
        closedCount: 0,
        pctOfGoal: null,
        commitAmount: 0,
...
function basePerformance(overrides: Partial<PerformanceMetrics> = {}): PerformanceMetrics {
    return {
        ...
        wonCount: 0,
        lostCount: 0,
        opportunities: { open: 0, createdInPeriod: 0, won: 0, lost: 0, advanced: 0, stalled: 0, eligible: 0, commit: 0, bestCase: 0, atRisk: 0 },
...
function baseLeadingIndicators(overrides: Partial<LeadingIndicatorsReport> = {}): LeadingIndicatorsReport {
    return {
...
function baseCrmQuality(overrides: Partial<CrmQualityIndex> = {}): CrmQualityIndex {
    return {
        ...
        evaluatedCount: 0,
        bitrixSync: {
            connected: false, totalOpen: 0, linked: 0, notLinked: 0, failed: 0, linkedRate: null,
            failures: [], lastSyncAt: null, syncedCount30d: 0, failedCount30d: 0,
        },
        ...overrides,
    };
}

function baseForecastAccuracy(overrides: Partial<ForecastAccuracySummary> = {}): ForecastAccuracySummary {
    return { available: false, reason: 'sem_historico_suficiente', sampleSize: 0, meanAbsoluteErrorPercent: null, samples: [], ...overrides };
}
...
describe('healthScore.computeHealthScore — agregação transparente em 6 pilares', () => {
    ...
    it('pilar Pipeline: coverage no nível recomendado = 100, classificado saudável', () => {
        const input = baseInput({
            overview: baseOverview({ coverageMonth: { coverage: 4, coverageRecommended: 4, pipelineEligible: 400_000, remainingGoal: 100_000 } }),
        });
    ...
    it('pilar Pipeline: sem meta cadastrada fica indisponível, não zero', () => {
        const input = baseInput({ overview: baseOverview({ goal: null, coverageMonth: { coverage: null, coverageRecommended: null, pipelineEligible: 0, remainingGoal: 0 } }) });
        const pipeline = computeHealthScore(input, NOW).pillars.find((p) => p.pillar === 'pipeline')!;
    ...
    it('pilar Conversão: usa o Win Rate diretamente como score e aplica os limiares documentados', () => {
        const saudavel = computeHealthScore(baseInput({ performance: basePerformance({ winRate: HEALTH_SCORE_RULES.WIN_RATE_HEALTHY_PCT }) }), NOW)
        ...
        const critico = computeHealthScore(baseInput({ performance: basePerformance({ winRate: 5 }) }), NOW)
            .pillars.find((p) => p.pillar === 'conversao')!;
        expect(critico.classification).toBe('critico');
    });

    it('pilar Conversão: sem negócio fechado no período fica indisponível', () => {
        const conversao = computeHealthScore(baseInput({ performance: basePerformance({ winRate: null }) }), NOW)
            .pillars.find((p) => p.pillar === 'conversao')!;
    ...
    it('pilar Produtividade: razão 1.0 (semana atual == média móvel) vira score 100 e classificação saudável', () => {
        const input = baseInput({
            leadingIndicators: baseLeadingIndicators({
                indicators: [
                    { label: 'Reuniões', current: 10, previousWeek: 10, movingAverage4w: 10, trend: 'flat', weeklySeries: [10, 10, 10, 10] },
                    { label: 'Propostas', current: 4, previousWeek: 4, movingAverage4w: 8, trend: 'down', weeklySeries: [8, 8, 8, 8] }, // razão 0.5
                ],
            }),
        });
        const produtividade = computeHealthScore(input, NOW).pillars.find((p) => p.pillar === 'produtividade')!;
    ...
    it('pilar Produtividade: nenhum indicador com média móvel > 0 fica indisponível', () => {
        const input = baseInput({
            leadingIndicators: baseLeadingIndicators({
                indicators: [{ label: 'Reuniões', current: 0, previousWeek: 0, movingAverage4w: 0, trend: 'flat', weeklySeries: [0, 0, 0, 0] }],
            }),
        });
        const produtividade = computeHealthScore(input, NOW).pillars.find((p) => p.pillar === 'produtividade')!;
    ...
    it('pilar Qualidade do CRM: reaproveita dataReadiness.overallScore/classification sem recalcular', () => {
        const input = baseInput({ crmQuality: baseCrmQuality({ dataReadiness: { overallScore: 62, classification: 'atencao', fields: [] } }) });
        const qualidade = computeHealthScore(input, NOW).pillars.find((p) => p.pillar === 'qualidadeCrm')!;
    ...
    it('pilar Follow-up: 100% do pipeline dentro do aging crítico vira score 100', () => {
        const input = baseInput({
            aging: baseAging({
                ...
                byStage: [{ stageId: 's1', stageName: 'Nova', count: 2, amountOverThreshold: 0, averageDaysInStage: 5, dataQuality: 'measured' }],
            }),
        });
    ...
    it('pilar Follow-up: metade do valor em risco vira score 50, classificado atenção (limiar de atenção documentado é 50)', () => {
        const input = baseInput({
            aging: baseAging({
                ...
                byStage: [{ stageId: 's1', stageName: 'Proposta', count: 2, amountOverThreshold: 100_000, averageDaysInStage: 120, dataQuality: 'measured' }],
            }),
        });
        const followUp = computeHealthScore(input, NOW).pillars.find((p) => p.pillar === 'followUp')!;
    ...
    it('pilar Follow-up: maior parte do valor em risco vira score baixo e crítico', () => {
        const input = baseInput({
            aging: baseAging({
                buckets: [{ label: '90+ dias', minDays: 91, maxDays: null, count: 2, amount: 200_000 }],
                byStage: [{ stageId: 's1', stageName: 'Proposta', count: 2, amountOverThreshold: 150_000, averageDaysInStage: 120, dataQuality: 'measured' }],
            }),
        });
        const followUp = computeHealthScore(input, NOW).pillars.find((p) => p.pillar === 'followUp')!;
    ...
    it('pilar Follow-up: sem negócio aberto (pipeline vazio) fica indisponível, não 100 fabricado', () => {
        const followUp = computeHealthScore(baseInput({ aging: baseAging({ buckets: [] }) }), NOW).pillars.find((p) => p.pillar === 'followUp')!;
    ...
    it('pilar Confiabilidade de Forecast: erro histórico baixo vira score alto', () => {
        const input = baseInput({ forecastAccuracy: baseForecastAccuracy({ available: true, reason: null, sampleSize: 3, meanAbsoluteErrorPercent: 10 }) });
        const confiabilidade = computeHealthScore(input, NOW).pillars.find((p) => p.pillar === 'confiabilidadeForecast')!;
    ...
    it('pilar Confiabilidade de Forecast: sem histórico de snapshot ainda fica "não disponível", nunca 0/100 fabricado', () => {
        const confiabilidade = computeHealthScore(baseInput(), NOW).pillars.find((p) => p.pillar === 'confiabilidadeForecast')!;
    ...
    it('overallScore é a média simples só dos pilares disponíveis, ignorando os indisponíveis', () => {
        const input = baseInput({
            ...
            crmQuality: baseCrmQuality({ dataReadiness: { overallScore: 80, classification: 'saudavel', fields: [] } }), // qualidadeCrm = 80

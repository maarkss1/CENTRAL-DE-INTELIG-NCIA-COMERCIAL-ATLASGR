...
import type { CommercialIntelligenceRepository, DealRow } from '../../domain/CommercialIntelligence';
...
export type StageHistoryRow = { leadId: string; stageId: string | null; stageName: string; enteredAt: Date; exitedAt: Date | null };
...
export function currentStageEntry(history: StageHistoryRow[], leadId: string, stageId: string | null): StageHistoryRow | null {
 ...
 */
export async function loadScoredDeals(repository: CommercialIntelligenceRepository, organizationId: string, now: Date): Promise<ScoredDealsResult> {

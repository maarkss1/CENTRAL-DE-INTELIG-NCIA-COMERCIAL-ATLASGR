 ...
 */
import type { ForecastAccuracyResult, ForecastAccuracySummary, ForecastSnapshotRecord, PeriodMonth } from '../domain/CommercialIntelligence';
...
function unavailable(period: PeriodMonth, reason: ForecastAccuracyResult['reason']): ForecastAccuracyResult {
    return {
        available: false,
        period,
        reason,
...
export function computeForecastAccuracy(
    ...
    const errorPercent = realizedClosedAmount > 0 ? roundMoney((Math.abs(errorAmount) / realizedClosedAmount) * 100) : null;
    const direction: ForecastAccuracyResult['direction'] = errorAmount > 0 ? 'superestimou' : errorAmount < 0 ? 'subestimou' : 'acertou';
 ...
 */
export function summarizeForecastAccuracy(samples: ForecastAccuracyResult[]): ForecastAccuracySummary {
    const usable = samples.filter((s): s is ForecastAccuracyResult & { errorPercent: number } => s.available && s.errorPercent != null);
    if (usable.length === 0) {
        return { available: false, reason: 'sem_historico_suficiente', sampleSize: 0, meanAbsoluteErrorPercent: null, samples };
    }
    const meanAbsoluteErrorPercent = roundMoney(usable.reduce((sum, s) => sum + s.errorPercent, 0) / usable.length);
    return { available: true, reason: null, sampleSize: usable.length, meanAbsoluteErrorPercent, samples };
}

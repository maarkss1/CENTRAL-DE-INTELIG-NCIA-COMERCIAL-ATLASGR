...
export { classifyCoverageProtection, COVERAGE_PROTECTION_FALLBACK_HEALTHY, COVERAGE_PROTECTION_FALLBACK_WARNING } from './coverageProtection';
...
export class CommercialIntelligenceUseCases {
    ...
    async setGoal(organizationId: string, period: string, amount: number, createdBy: string, currency = 'BRL', metric: GoalMetric = 'NEW_MRR'): Promise<CommercialGoalDTO> {
        return setGoalCommand(this.repository, organizationId, period, amount, createdBy, currency, metric);
    }
    ...
    async forecastExplain(organizationId: string, leadId: string, now = new Date()): Promise<ForecastExplain | null> {

    ...
    async countSentTouchesForContact(organizationId: string, leadId: string, since: Date, until: Date): Promise<number> {
        ...
        for (const run of runs) {
            ...
            for (const attempt of run.attempts) {
                if (attempt.result === 'sent' && attempt.attemptedAt >= since && attempt.attemptedAt <= until) count++;
            }
        }
        return count;
    }
    ...
    async countDistinctEmailRecipientsForDomain(
        ...
        for (const run of runs) {
            ...
            const hasSentEmailInWindow = run.attempts.some(
                (a) => a.channel === 'email' && a.result === 'sent' && a.attemptedAt >= since && a.attemptedAt <= until,
            );

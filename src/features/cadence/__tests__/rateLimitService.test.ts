...
import { startCadenceRun, recordTouchAttempt, type CadenceSequenceDefinition, type CadenceTouch, type CadenceRunState } from '../domain/cadence';
...
function seedPriorSentTouches(
    ...
    input: { runId: string; leadId: string; channel: 'email' | 'whatsapp' | 'voice'; count: number; sentAt: Date },
): void {
    let run: CadenceRunState = startCadenceRun({ id: input.runId, organizationId: ORG, leadId: input.leadId, sequenceId: `seq-historico-${input.runId}`, startedAt: input.sentAt });
    const historicalSequence: CadenceSequenceDefinition = {
        id: `seq-historico-${input.runId}`,
        name: 'histórico',
        touches: Array.from({ length: input.count }, (_, i) => ({ order: i + 1, channel: input.channel, delayHoursFromPrevious: 0 })),
    };
    for (let i = 0; i < input.count; i++) {
        run = recordTouchAttempt(run, historicalSequence, historicalSequence.touches[i], input.sentAt, { result: 'sent' });
    }
    runRepo.seed(run);
}
...
describe('rate limit por contato — no máximo N toques (default 3) em 24h, qualquer canal/cadência', () => {
    ...
    it('(a) contato já no limite (3 toques em outra cadência, últimas 24h) é bloqueado — decisão wait/contact-rate-limit, dispatcher nunca chamado', async () => {
        ...
        seedPriorSentTouches(runRepo, { runId: 'run-historico', leadId: LEAD, channel: 'whatsapp', count: 3, sentAt: new Date(NOW.getTime() - 3_600_000) });

        const newRun = startCadenceRun({ id: 'run-novo', organizationId: ORG, leadId: LEAD, sequenceId: EMAIL_TOUCH_SEQUENCE.id, startedAt: NOW });
        runRepo.seed(newRun);
        ...
        expect(updated.attempts[0]).toMatchObject({ result: 'skipped', skipReason: 'contact-rate-limit', touchOrder: 1 });
    ...
    it('(b) contato abaixo do limite (2 toques anteriores) passa normalmente — dispatcher chamado, toque avança', async () => {
        ...
        seedPriorSentTouches(runRepo, { runId: 'run-historico', leadId: LEAD, channel: 'whatsapp', count: 2, sentAt: new Date(NOW.getTime() - 3_600_000) });
    ...
    it('toques fora da janela de 24h não contam para o limite', async () => {
        ...
        seedPriorSentTouches(runRepo, { runId: 'run-historico', leadId: LEAD, channel: 'whatsapp', count: 3, sentAt: new Date(NOW.getTime() - 48 * 3_600_000) });
    ...
    it('bloqueio repetido no mesmo motivo não acumula uma linha nova de tentativa a cada ciclo (evita ruído no histórico)', async () => {
        ...
        const { run: afterSecondTick } = await advanceCadenceRun(deps, ORG, 'run-novo', EMAIL_TOUCH_SEQUENCE, fiveMinutesLater);
...
describe('rate limit por domínio — no máximo M contatos distintos recebendo e-mail no mesmo dia', () => {
    ...
    it('(c) domínio no limite bloqueia um NOVO contato desse domínio no canal e-mail', async () => {
        ...
        seedPriorSentTouches(runRepo, { runId: 'run-a', leadId: LEAD_A, channel: 'email', count: 1, sentAt: new Date(NOW.getTime() - 3_600_000) });

        const runB = startCadenceRun({ id: 'run-b', organizationId: ORG, leadId: LEAD_B, sequenceId: EMAIL_TOUCH_SEQUENCE.id, startedAt: NOW });
        runRepo.seed(runB);
        ...
        expect(dispatcher.calls).toHaveLength(0);
        expect(updated.attempts[0]).toMatchObject({ result: 'skipped', skipReason: 'domain-rate-limit' });
    });
    ...
    it('domínio no limite NÃO bloqueia canal diferente de e-mail para o mesmo contato novo', async () => {
        ...
        const runB = startCadenceRun({ id: 'run-b', organizationId: ORG, leadId: LEAD_B, sequenceId: WHATSAPP_TOUCH_SEQUENCE.id, startedAt: NOW });
        runRepo.seed(runB);
    ...
    it('domínio no limite NÃO afeta um contato de domínio DIFERENTE', async () => {
        ...
        const runC = startCadenceRun({ id: 'run-c', organizationId: ORG, leadId: LEAD_C, sequenceId: EMAIL_TOUCH_SEQUENCE.id, startedAt: NOW });
        runRepo.seed(runC);
    ...
    it('um contato que já é o próprio destinatário contado hoje não é bloqueado de novo por domain-rate-limit (não é um destinatário NOVO)', async () => {
        ...
        seedPriorSentTouches(runRepo, { runId: 'run-a-historico', leadId: LEAD_A, channel: 'email', count: 1, sentAt: new Date(NOW.getTime() - 3_600_000) });

        const runNovo = startCadenceRun({ id: 'run-a-novo', organizationId: ORG, leadId: LEAD_A, sequenceId: EMAIL_TOUCH_SEQUENCE.id, startedAt: NOW });
...
describe('evaluateRateLimitForUpcomingTouch — política customizada via parâmetro (não hardcoded)', () => {
    it('usa a política passada em vez do default quando fornecida', async () => {
        ...
        seedPriorSentTouches(runRepo, { runId: 'run-historico', leadId: LEAD, channel: 'email', count: 1, sentAt: new Date(NOW.getTime() - 3_600_000) });
        const rateLimit = new InMemoryCadenceRateLimitPort(runRepo, () => null);

        const tightPolicy: CadenceRateLimitPolicy = { maxTouchesPerContactWindow: 1, contactWindowHours: 24, maxEmailRecipientsPerDomainPerDay: 20 };

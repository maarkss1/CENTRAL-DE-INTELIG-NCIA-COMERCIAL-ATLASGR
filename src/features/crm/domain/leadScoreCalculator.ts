export interface BantQualificationData {
    ...
    authority?: 'decisor_clevel' | 'influenciador_gerente' | 'usuario_operacional' | 'sem_autoridade' | string;
    need?: 'critica_urgente' | 'moderada_otimizacao' | 'curiosidade_benchmarking' | 'sem_dor' | string;
...
export function calculateLeadScore(data: BantQualificationData = {}): LeadScoreResult {
    ...
    const totalScore = Math.min(100, Math.max(0, budgetScore + authorityScore + needScore + timingScore));
    ...
    if (totalScore >= 70) {
        temperature = 'Quente';
        recommendation = 'Alta propensão de fechamento! Agendar demonstração técnica ou enviar proposta comercial.';
    } else if (totalScore >= 40) {
        temperature = 'Morno';
        recommendation = 'Lead qualificado com potencial. Aprofundar diagnóstico de ROI e validar orçamento.';
    }

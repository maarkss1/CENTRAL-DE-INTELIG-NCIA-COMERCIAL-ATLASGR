...
import { X, Download, Search, Loader2, CheckSquare, Square, Building2, User, Phone, AlertCircle, RefreshCw } from 'lucide-react';
import { bitrixApi, type BitrixConnectionItem, type BitrixLeadItem, type BitrixDealItem } from '../../integrations/bitrix/bitrix.api';
...
export function BitrixImportModal({ isOpen, onClose, onImportSuccess }: BitrixImportModalProps) {
    ...
    const fetchRecords = useCallback(async () => {
        ...
        try {
            if (entityType === 'lead') {
                const data = await bitrixApi.listLeads(selectedConnectionId, searchTerm.trim() || undefined);
                setLeads(data.leads || []);
            } else {
                const data = await bitrixApi.listDeals(selectedConnectionId, searchTerm.trim() || undefined);
                setDeals(data.deals || []);
            }
        } catch (err) {
    ...
    const handleImportSelected = async () => {
        ...
        try {
            ...
            toast.success(`${ids.length} ${entityType === 'lead' ? 'lead(s)' : 'negócio(s)'} importado(s) com sucesso!`);
            onImportSuccess();
            onClose();
        } catch (err) {
    ...
    const handleQuickSyncRecent = async () => {
        ...
        try {
            ...
            toast.success(`Sincronização concluída: ${res.data.imported} importados, ${res.data.skipped} já existentes.`);
            onImportSuccess();
            onClose();
        } catch (err) {
            ...
            <div className="bg-surface border border-line rounded-3xl w-full max-w-3xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden">
                ...
                <div className="p-6 border-b border-line flex items-center justify-between bg-surface-2/60">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-ink-2">Selecione os leads ou negócios do portal Bitrix para importar sob demanda para o CRM</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-6 border-b border-line space-y-4 bg-surface">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-ink-2 mb-1.5 uppercase">Portal Bitrix Conectado</label>
                            <select
                        ...
                        <div>
                            <label className="block text-xs font-bold text-ink-2 mb-1.5 uppercase">Objeto do Bitrix</label>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-3 custom-scrollbar">
                    {loading ? (
                        ...
                        <div className="py-12 text-center text-ink-2 space-y-2">
                            ...
                            <p className="text-xs">Configure o webhook do Bitrix na aba Integrações para habilitar a importação.</p>
                        </div>
                        ...
                        <>
                            <div className="flex items-center justify-between pb-2 border-b border-line text-xs text-ink-2">
                                ...
                                >
                                    ...
                                    {selectedIds.size === (entityType === 'lead' ? leads : deals).length ? 'Desmarcar todos' : 'Selecionar todos'}
                                </button>
                                <span>{selectedIds.size} de {(entityType === 'lead' ? leads : deals).length} selecionado(s)</span>
                            </div>
                                            ...
                                            >
                                                <div className="flex items-center gap-3 min-w-0">
                                                    <div className="text-brand shrink-0">
                                                        {isSelected ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4 text-ink-2" />}
                                                    </div>
                                                    <div className="min-w-0">
                                                        <div className="flex items-center gap-2">
                                                            <span className="font-bold text-xs text-ink truncate">{l.title || l.companyTitle || `Lead #${l.id}`}</span>
                                                        ...
                                                        <div className="flex items-center gap-3 mt-1 text-[11px] text-ink-2">
                                                            {l.companyTitle && (
                                                                <span className="flex items-center gap-1 truncate"><Building2 className="w-3 h-3 text-brand" /> {l.companyTitle}</span>
                                                            )}
                                                            {l.name && (
                                                                <span className="flex items-center gap-1 truncate"><User className="w-3 h-3" /> {l.name} {l.lastName || ''}</span>
                                                            )}
                                                            {l.phone && (
                                                                <span className="flex items-center gap-1 truncate"><Phone className="w-3 h-3" /> {l.phone}</span>
                                                            )}
                                                        </div>
                                                    </div>
                                                </div>
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="flex items-center gap-3 min-w-0">
                                                    ...
                                                    <div className="min-w-0">
                                                        <div className="flex items-center gap-2">
                                                            <span className="font-bold text-xs text-ink truncate">{d.title || `Negócio #${d.id}`}</span>
                                                            <span className="text-[10px] px-2 py-0.5 rounded-full bg-surface border border-line text-ink-2 font-medium">
                                                        ...
                                                        <div className="flex items-center gap-3 mt-1 text-[11px] text-ink-2">
                                                            {d.companyTitle && (
                                                                <span className="flex items-center gap-1 truncate"><Building2 className="w-3 h-3 text-brand" /> {d.companyTitle}</span>
                                                            )}
                                                            {d.contactName && (
                                                                <span className="flex items-center gap-1 truncate"><User className="w-3 h-3" /> {d.contactName}</span>
                                                            )}
                                                        </div>
                                                    </div>
                                                </div>
                ...
                <div className="p-6 border-t border-line bg-surface-2/60 flex flex-col sm:flex-row items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-3 w-full sm:w-auto">
                        ...
                        >
                            {importing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4 rotate-180" />}

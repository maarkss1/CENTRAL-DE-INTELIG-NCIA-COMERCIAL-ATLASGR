            ...
            <div className="bg-surface border border-line rounded-3xl shadow-2xl max-w-2xl w-full p-6 space-y-6 max-h-[85vh] flex flex-col">
                <div className="flex items-center justify-between border-b border-line pb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-ink-2">Automatize buscas periódicas de novos leads e frotistas</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="flex-1 overflow-y-auto space-y-4 pr-1">
                    ...
                    {!showCreateForm ? (
                        ...
                        <form onSubmit={handleCreate} className="p-4 rounded-2xl bg-surface-2/60 border border-line space-y-3">
                            ...
                            <div>
                                <label className="text-xs font-medium text-ink block mb-1">Recorrência Automática</label>
                                <select
                                    value={newSchedule}
                            ...
                            <div className="flex justify-end gap-2 pt-2">
                                ...
                                >
                                    {creating ? <Loader2 size={14} className="animate-spin" /> : <Sparkles size={14} />} Salvar Lista
                                </button>
                            </div>
                        </form>
                    )}
                            ...
                            >
                                <div className="space-y-1">
                                    ...
                                    <div className="flex items-center gap-3 text-[11px] text-ink-2">
                                        <span>Total Descobertos: <b className="text-ink">{s.leadsDiscovered}</b></span>
                                        {s.lastRunAt && (
                                            <span>Última execução: {new Date(s.lastRunAt).toLocaleDateString('pt-BR')}</span>
                                        )}
                                    </div>
                                </div>

...
function stepFor(steps: ReturnType<typeof planCompanyDiscovery>['steps'], provider: 'apollo' | 'googlePlaces' | 'nominatim') {
...
describe('scoreProvider', () => {
    ...
    it('bônus de filtro firmográfico só se aplica à Apollo (único provider que sabe filtrar por isso)', () => {
        ...
        expect(scoreProvider('googlePlaces', intent).score).toBe(scoreProvider('googlePlaces', baseIntent()).score);
        expect(scoreProvider('nominatim', intent).score).toBe(scoreProvider('nominatim', baseIntent()).score);
    });

    it('bônus de precisão geográfica se aplica a Google Places e Nominatim, não à Apollo', () => {
        const intent = baseIntent({ location: { label: 'Niterói, RJ', city: 'Niterói', state: 'RJ', excluded: [], isCitySpecific: true } });
        expect(scoreProvider('apollo', intent).score).toBe(scoreProvider('apollo', baseIntent()).score);
        expect(scoreProvider('googlePlaces', intent).score).toBeGreaterThan(scoreProvider('googlePlaces', baseIntent()).score);
        expect(scoreProvider('nominatim', intent).score).toBeGreaterThan(scoreProvider('nominatim', baseIntent()).score);
    });

    it('bônus de decisor pré-buscado só se aplica à Apollo', () => {
        const intent = baseIntent({ needsDecisionMakerContacts: true, decisionMakerTitles: ['Diretor de Logística'] });
        expect(scoreProvider('apollo', intent).score).toBeGreaterThan(scoreProvider('apollo', baseIntent()).score);
        expect(scoreProvider('googlePlaces', intent).score).toBe(scoreProvider('googlePlaces', baseIntent()).score);
    });
...
describe('planCompanyDiscovery — quais providers e em que ordem', () => {
    ...
    it(`quantidade > ${NOMINATIM_SUPPLEMENT_MIN_QUANTITY}: Nominatim entra no plano`, () => {
        const plan = planCompanyDiscovery(baseIntent({ quantityRequested: NOMINATIM_SUPPLEMENT_MIN_QUANTITY + 1 }), 'hybrid');
...
describe('planCompanyDiscovery — cotas (aritmética preservada do cascade legado)', () => {
    it('Apollo sempre pede a quantidade total, independente de cidade/geografia', () => {
        ...
        const withCity = planCompanyDiscovery(
            baseIntent({ quantityRequested: 20, location: { label: 'Niterói, RJ', city: 'Niterói', state: 'RJ', excluded: [], isCitySpecific: true } }),
            'hybrid'
        );
    ...
    it('Google Places pede min(total, 25) quando a busca NÃO é hiperlocal', () => {
        expect(stepFor(planCompanyDiscovery(baseIntent({ quantityRequested: 20 }), 'hybrid').steps, 'googlePlaces')?.quota).toBe(20);
        expect(stepFor(planCompanyDiscovery(baseIntent({ quantityRequested: 3 }), 'hybrid').steps, 'googlePlaces')?.quota).toBe(3);
    });
    ...
    it('Google Places reserva ~40% da cota (arredondado, mínimo 1) quando a busca é hiperlocal (cidade+estado)', () => {
        const cityIntent = (quantityRequested: number) =>
            baseIntent({ quantityRequested, location: { label: 'Niterói, RJ', city: 'Niterói', state: 'RJ', excluded: [], isCitySpecific: true } });

        expect(stepFor(planCompanyDiscovery(cityIntent(20), 'hybrid').steps, 'googlePlaces')?.quota).toBe(8); // round(20*0.4)
        expect(stepFor(planCompanyDiscovery(cityIntent(5), 'hybrid').steps, 'googlePlaces')?.quota).toBe(2); // round(5*0.4)
        expect(stepFor(planCompanyDiscovery(cityIntent(1), 'hybrid').steps, 'googlePlaces')?.quota).toBe(1); // nunca cai a 0
    });

    it('Nominatim pede min(total, 20) quando entra no plano', () => {
        expect(stepFor(planCompanyDiscovery(baseIntent({ quantityRequested: 20 }), 'hybrid').steps, 'nominatim')?.quota).toBe(20);
        expect(stepFor(planCompanyDiscovery(baseIntent({ quantityRequested: 16 }), 'hybrid').steps, 'nominatim')?.quota).toBe(16);
    });
});

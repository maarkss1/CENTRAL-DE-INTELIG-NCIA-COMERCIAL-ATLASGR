...
export async function findEmailViaHunter(domain: string, fullName: string): Promise<HunterEmailResult> {
    ...
    try {
        ...
        const res = await fetchWithProviderRetry(`https://api.hunter.io/v2/email-finder?${params.toString()}`, {}, { timeoutMs: 12_000, providerName: 'Hunter-EmailFinder' });
...
export async function findPeopleViaDomainSearch(
    ...
    try {
        ...
        const res = await fetchWithProviderRetry(`https://api.hunter.io/v2/domain-search?${params.toString()}`, {}, { timeoutMs: 12_000, providerName: 'Hunter-DomainSearch' });
        if (!res.ok) {

...
export function BookingLinksModal({ isOpen, onClose }: BookingLinksModalProps) {
    ...
    const [form, setForm] = useState({
        ...
        slug: 'demo-atlas',
        durationMin: 30,
        description: 'Apresentação prática da central de inteligência, telemetria CAN e redução de custos operacionais.',
    });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-surface border border-line rounded-3xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden">
                ...
                <div className="p-6 border-b border-line flex items-center justify-between bg-surface-2/60">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-ink-2">Páginas de agendamento estilo Calendly integradas diretamente à sua agenda e CRM</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="p-2 rounded-xl text-ink-2 hover:text-ink hover:bg-surface-2 transition-colors">
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                    ...
                    <form onSubmit={handleCreate} className="bg-surface-2/50 p-5 rounded-2xl border border-line space-y-3.5">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                <label className="block text-[11px] font-bold text-ink-2 mb-1">Título da Reunião</label>
                                <input
                                    type="text"
                                    value={form.title}
                            ...
                            <div>
                                <label className="block text-[11px] font-bold text-ink-2 mb-1">Link Personalizado (/book/...)</label>
                                <div className="flex items-center">
                                    <span className="text-xs text-ink-2 bg-surface-2 px-2.5 py-2 border border-r-0 border-line rounded-l-xl">/book/</span>
                                    <input
                                        type="text"
                                        value={form.slug}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                <label className="block text-[11px] font-bold text-ink-2 mb-1">Duração (minutos)</label>
                                <select
                                    ...
                                    >
                                        <div className="space-y-1 min-w-0">
                                            <div className="flex items-center gap-2">
                                                <span className="font-extrabold text-xs text-ink truncate">{link.title}</span>

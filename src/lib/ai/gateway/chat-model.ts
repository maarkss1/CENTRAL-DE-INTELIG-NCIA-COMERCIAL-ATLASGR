...
function buildExhaustedProvidersError(errorsByProvider: Map<string, unknown>): Error {
    const configuredNames: Record<string, string> = { groq: 'Groq', openai: 'OpenAI', litellm: 'Ollama/LiteLLM' };
    const configured = PROVIDER_CHAIN
        .filter((provider) => provider.isConfigured())
        .map((provider) => configuredNames[provider.name])
        .join(', ');

    if (!configured) {
        return new Error('Nenhum motor de IA configurado. Defina GROQ_API_KEY (gratuito, console.groq.com) ou OPENAI_API_KEY no .env.');
    }
...
export const getAiModel = (modelName: string = 'local-llama3', temperature: number = 0.7, agentContext: string = 'system'): AiChatModel => {
    ...
    return {
        async invoke(messages: BaseMessage[]): Promise<AiInvokeResult> {
            ...
            const { response, providerUsed } = await callWithFallback(resolvedModel, requestMessages, temperature, agentContext);

...
export function readBoundedInteger(value: string | undefined, fallback: number, min: number, max: number): number {
...
export function resolveFallbackTimeoutMs(): number {
    return readBoundedInteger(process.env.AI_FALLBACK_TIMEOUT_MS, DEFAULT_FALLBACK_TIMEOUT_MS, 5_000, 120_000);
}
...
export function resolveEmbeddingTimeoutMs(): number {
    return readBoundedInteger(process.env.AI_EMBEDDING_TIMEOUT_MS, DEFAULT_GATEWAY_TIMEOUT_MS, 5_000, 120_000);
}

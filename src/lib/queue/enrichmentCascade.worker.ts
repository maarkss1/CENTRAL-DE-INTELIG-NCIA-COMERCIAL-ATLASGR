...
import { runEnrichmentCascade, type CascadeEnrichmentOptions } from '../../features/prospecting/services/enrichmentCascade.service.js';
...
if (enrichmentCascadeQueue) {
    ...
    enrichmentCascadeQueue.on('error', (err) => logger.warn({ message: err.message }, 'enrichmentCascadeQueue offline'));
}
...
export function createEnrichmentCascadeWorker() {
    const worker = new Worker<EnrichmentCascadeJobData>(
        ...
        async (job: Job<EnrichmentCascadeJobData>) => {
            ...
            logger.info({ jobId: job.id, companyId, organizationId }, 'Iniciando job de enriquecimento em cascata');
            ...
            await requestContext.run({ tenantId: organizationId }, async () => {
                try {
                    ...
                    logger.error({ err: error, jobId: job.id, companyId }, 'Falha no job de enriquecimento em cascata');
                    throw error;
                }
            });
        },

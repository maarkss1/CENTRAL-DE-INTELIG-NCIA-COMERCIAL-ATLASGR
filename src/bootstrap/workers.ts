...
export function startEmbeddedWorkers(): EmbeddedWorkersHandle {
    ...
    enabledOrganizations().then((coldCallOrgs) => {
        handle.coldCallWorker = embeddedWorkersEnabled && coldCallOrgs.length > 0 ? createColdCallWorker() : null;
    ...
    swarmSchedulerEnabledOrganizations().then((swarmOrgs) => {
        handle.swarmSchedulerWorker = embeddedWorkersEnabled && swarmOrgs.length > 0 ? createSwarmSchedulerWorker() : null;

...
import {
    Search, CornerDownLeft, ArrowUp, ArrowDown, Building2, Users, Activity,
    FileBarChart, Bot, Loader2,
} from 'lucide-react';
...
import { OPEN_COMMAND_PALETTE_EVENT, OPEN_AI_CHAT_EVENT, type PaletteIntent } from '../../lib/paletteIntent';
...
export function CommandPalette() {
        ...
        const timer = window.setTimeout(async () => {
            try {
                const [companyRes, contactRes] = await Promise.all([
                    api.get<PaginatedResponse<Company>>(`/api/companies?q=${encodeURIComponent(trimmed)}&limit=5`),
                    api.get<PaginatedResponse<Contact>>(`/api/contacts?q=${encodeURIComponent(trimmed)}&limit=5`),
                ]);
    ...
    const navigateAndClose = useCallback((tab: TabType, intent?: PaletteIntent) => {
    ...
    const items = useMemo<ResultItem[]>(() => {
        ...
        const quickActions: ResultItem[] = [
            {
                id: 'qa-activity', group: 'Ações rápidas', label: 'Criar nova atividade', icon: Activity,
                onSelect: () => navigateAndClose('activities', { type: 'open-create' }),
            },
            {
                id: 'qa-prospect', group: 'Ações rápidas', label: 'Iniciar prospecção', icon: Search,
                onSelect: () => navigateAndClose('prospect'),
            },
            {
                id: 'qa-report', group: 'Ações rápidas', label: 'Abrir Relatórios de IA', icon: FileBarChart,
                onSelect: () => navigateAndClose('reports'),
            },
            {
                id: 'qa-ai', group: 'Ações rápidas', label: 'Chamar copiloto de IA', icon: Bot,
                onSelect: () => { window.dispatchEvent(new Event(OPEN_AI_CHAT_EVENT)); close(); },
            },
        ];
        ...
        if (q.length >= 2) {
            result.push(...companies.map((c) => ({
                id: `company-${c.id}`,
                group: 'Empresas' as const,
                label: c.tradeName || c.legalName,
                sublabel: c.city ? `${c.city}${c.state ? `/${c.state}` : ''}` : undefined,
                icon: Building2,
                onSelect: () => navigateAndClose('companies', { type: 'prefill-search', value: c.tradeName || c.legalName }),
            })));
            result.push(...contacts.map((c) => ({
                id: `contact-${c.id}`,
                        ...
                        return (
                            <div key={item.id}>
                                ...
                                >
                                    ...
                                    {item.sublabel && <span className="shrink-0 truncate text-xs text-ink-2">{item.sublabel}</span>}

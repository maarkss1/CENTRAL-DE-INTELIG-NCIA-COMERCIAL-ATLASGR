...
// dois tenants (ORG_A/ORG_B) na mesma conexão de propósito — então usa bypassRls (ver
// src/lib/async-context.ts) só pra essas operações de setup/assert/teardown não serem bloqueadas
// pelas mesmas policies de FORCE ROW LEVEL SECURITY que valem em produção. Isso não invalida o que
...
describe('Isolamento de tenant — KnowledgeChunk, Prompt, AgentMemory, AIPendingAction', () => {
  ...
  it('KnowledgeChunk: busca por organizationId nunca retorna trecho de outro tenant', async () => withRlsBypass(async () => {
  ...
  it('Prompt: listagem e edição por organizationId nunca expõem/alteram prompt de outro tenant (IDOR corrigido)', async () => withRlsBypass(async () => {
  ...
  it('AgentMemory: findFirst por sessionId+organizationId nunca resolve a memória de outro tenant', async () => withRlsBypass(async () => {
  ...
  it('AIPendingAction: rotas de aprovar/descartar por id+organizationId não afetam ação de outro tenant', async () => withRlsBypass(async () => {
